...
class AudioResampler(torch.nn.Module):
    ...
    def __init__(
        ...
        times = (
            ...
            - (
                np.arange(kernel_width, dtype=np_dtype).reshape((1, 1, kernel_width))
                - blocks_per_side
            )
        )
        ...
        def kaiser_window(a, beta):
            w = special.i0(
                beta * np.sqrt(np.clip(1 - ((a - 0.0) / 1.0) ** 2.0, 0.0, 1.0))
            ) / special.i0(beta)
        ...
        if output_sr == 1:
            ...
            self.weights = (
                weights.transpose(1, 2).contiguous().view(1, 1, input_sr * kernel_width)
            )
            ...
            self.weights = (
                ...
                .transpose(0, 2)
                .contiguous()
                .view(1, 1, output_sr * kernel_width)
            )
        else:
    ...
    def forward(self, data):
        ...
        if self.resample_type == "trivial":
            ...
            data = torch.nn.functional.conv1d(
                data, self.weights, stride=self.input_sr, padding=self.padding
            )
            ...
            data = torch.nn.functional.conv_transpose1d(
                data, self.weights, stride=self.output_sr, padding=self.padding
            )
            ...
            return (
                data.transpose(1, 2)
                .contiguous()
                .view(minibatch_size, num_blocks * self.output_sr)
            )
