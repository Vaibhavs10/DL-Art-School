...
def music2mel(clip):
    ...
    inj = TorchMelSpectrogramInjector(
        {
            "n_mel_channels": 256,
            "mel_fmax": 11000,
            "filter_length": 16000,
...
def get_mel2wav_model():
    ...
    model = DiffusionWaveformGen(
        ...
        num_res_blocks=[3, 3, 2, 2, 1],
        token_conditioning_resolutions=[1, 4, 16],
        dropout=0,
        kernel_size=3,
        scale_factor=2,
    ...
    model.load_state_dict(
        torch.load("../experiments/music_mel2wav.pth", map_location=torch.device("cpu"))
    )
    model.eval()
    return model
...
def get_mel2wav_v3_model():
    ...
    model = DiffusionWaveformGen(
        ...
        in_mel_channels=256,
        out_channels=32,
        channel_mult=[1, 1.5, 2, 4],
        num_res_blocks=[2, 1, 1, 0],
        mid_resnet_depth=24,
...
def get_music_codegen():
    ...
    model.load_state_dict(
        torch.load(f"../experiments/m2v_music.pth", map_location=torch.device("cpu"))
    )
    model = model.eval()
    return model
...
def get_cheater_decoder():
    ...
    model.load_state_dict(
        torch.load(
            f"../experiments/music_cheater_decoder.pth",
            map_location=torch.device("cpu"),
        )
    )
    model = model.eval()
