...
def test_stability(mod_fn, dummy_inputs, device="cuda"):
    ...
    for k in means.keys():
        print(
            "%s - mean: %f std: %f"
            % (k, torch.mean(torch.stack(means[k])), torch.mean(torch.stack(stds[k])))
        )


def test_stability_per_module(
    mod: nn.Module, input_shapes: dict, device="cuda", id="base"
):
    means = {}
    stds = {}
...
if __name__ == "__main__":
    ...
    """
    test_stability(
        functools.partial(disc.Discriminator_UNet_FeaOut, 3, 64),
