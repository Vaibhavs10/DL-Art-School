...
def get_ctc_codes_for(src_clip_path):
    ...
    clip_inp = processor(
        clip.numpy(), return_tensors="pt", sampling_rate=16000
    ).input_values.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-src_clip",
        type=str,
        help="Path to the audio files to translate",
        default="D:\\tortoise-tts\\voices",
    )
    parser.add_argument(
        "-opt",
        type=str,
    ...
    parser.add_argument(
        "-voice", type=str, help="Type of conditioning voice", default="plants"
    )
    parser.add_argument(
        "-diffusion_steps",
        type=int,
    ...
    with torch.no_grad():
        ...
        for e, file in enumerate(files):
            ...
            torchaudio.save(
                os.path.join(args.output_path, f"{e}_source_clip.wav"),
                src_clip.unsqueeze(0).cpu(),
                16000,
            )
            ...
            torchaudio.save(
                os.path.join(args.output_path, f"{e}_output_mean_base.wav"),
            ...
            output = sr_diffuser.p_sample_loop(
                sr_diffusion,
                output_shape,  # noise=torch.zeros(output_shape, device=args.device),
                model_kwargs={
                    "tokens": aligned_codes,
            ...
            torchaudio.save(
                os.path.join(args.output_path, f"{e}_output_mean_sr.wav"),
