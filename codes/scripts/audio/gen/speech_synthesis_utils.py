...
def load_speech_dvae():
    dvae = load_model_from_config(
        "../experiments/train_diffusion_vocoder_22k_level.yml", "dvae"
    ).cpu()
    dvae.eval()
    return dvae
...
def load_clvp():
    ...
    clvp.load_state_dict(
        torch.load(f"../experiments/clvp_md.pth", map_location=torch.device("cpu"))
    )
    clvp = clvp.eval()
    return clvp
...
def wav_to_mel(wav, mel_norms_file="../experiments/clips_mel_norms.pth"):
    ...
    """
    return TorchMelSpectrogramInjector(
        {"in": "wav", "out": "mel", "mel_norm_file": mel_norms_file}, {}
    )({"wav": wav})["mel"]
...
def load_gpt_conditioning_inputs_from_directory(
    path, num_candidates=3, sample_rate=22050, max_samples=44100
):
    candidates = find_files_of_type(
        "img", os.path.dirname(path), qualifier=is_audio_file
    )[0]
    assert (
        len(candidates) < 50000
...
def load_discrete_vocoder_diffuser(
    ...
    return SpacedDiffusion(
        ...
        loss_type="mse",
        betas=get_named_beta_schedule(schedule, trained_diffusion_steps),
        conditioning_free=enable_conditioning_free_guidance,
...
def do_spectrogram_diffusion(
    ...
    mel_codes,
    conditioning_input,
    spectrogram_compression_factor=128,
    plt_spec=False,
    mean=False,
):
    """
    ...
    with torch.no_grad():
        ...
        if mean:
            return diffuser.p_sample_loop(
                diffusion_model,
                output_shape,
                noise=torch.zeros(output_shape, device=mel_codes.device),
                model_kwargs={
                    "spectrogram": mel,
