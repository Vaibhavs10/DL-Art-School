...
def join_music_with_cheaters(clip1_cheater, clip2_cheater, results_dir):
    ...
    diffuser = SpacedDiffusion(
        use_timesteps=space_timesteps(4000, [256]),
        model_mean_type="epsilon",
        model_var_type="learned_range",
        loss_type="mse",
        betas=get_named_beta_schedule(diffusion_type, 4000),
        conditioning_free=True,
        conditioning_free_k=2,
    )

    inp = torch.cat(
        [clip1_leadin, torch.zeros(1, 256, 240, device="cuda"), clip2_leadin], dim=-1
    )
    mask = torch.ones_like(inp)
    mask[:, :, 60:-60] = 0
    gen_cheater = diffuser.ddim_sample_loop_with_guidance(
        model,
        inp,
        mask,  # causal=True, causal_slope=4,
    ...
    cheater_decoder_diffuser = SpacedDiffusion(
        use_timesteps=space_timesteps(4000, [64]),
        model_mean_type="epsilon",
        model_var_type="learned_range",
        loss_type="mse",
    ...
    for i, chunk_cheater in enumerate(tqdm(chunks)):
        gen_mel = cheater_decoder_diffuser.ddim_sample_loop(
            cheater_to_mel,
            (1, 256, chunk_cheater.shape[-1] * 16),
            progress=True,
        ...
        wav = spectral_diffuser.ddim_sample_loop(
            m2w, output_shape, progress=True, model_kwargs={"codes": gen_mel_denorm}
        )
...
if __name__ == "__main__":
    ...
    os.makedirs(results_dir, exist_ok=True)
    cheater = torch.tensor(
        np.load(
            "Y:\\separated\\large_mel_cheaters\\bt-music-1\\ff8-fithos_lusec_wecos_vinosec-1999\\08 - The Man with the Machine Gun\\0.npz"
        )["arr_0"]
    ).cuda()
