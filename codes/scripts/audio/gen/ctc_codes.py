...
def diffuse(text, codes, cond):
    ...
    diffusion_model = TransformerDiffusionTTS(
        model_channels=896,
        num_layers=16,
        in_channels=100,
    ...
    with torch.no_grad():
        text_codes = (
            torch.LongTensor(bpe_tokenizer.encode(text)).unsqueeze(0).to(codes.device)
        )
        ...
        gen = diffuser.p_sample_loop(
            diffusion_model,
            (1, 100, int(codes.shape[-1] * RATIO)),
            model_kwargs={
                "codes": codes,
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        decoded_codes = decode_ctc_metadata(codes[0], ppads[0], prepeats[0]).unsqueeze(
            0
        )
        cond = (
            load_audio(
                "D:\\tortoise-tts\\tortoise\\voices\\train_dotrice\\1.wav", 22050
            )
            .unsqueeze(0)
            .cuda()
        )
