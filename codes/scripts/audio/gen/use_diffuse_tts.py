...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-opt_code_gen",
        type=str,
        help="Path to options YAML file used to train the code_gen model",
        default="D:\\dlas\\options\\train_encoder_build_ctc_alignments.yml",
    )
    parser.add_argument(
        "-code_gen_model_name",
        type=str,
        help="Name of the code_gen model in opt.",
        default="generator",
    )
    parser.add_argument(
        "-code_gen_model_path",
        type=str,
        help="Path to saved code_gen model weights",
    ...
    parser.add_argument(
        "-sr_opt",
        type=str,
        help="Path to options YAML file used to train the SR diffusion model",
        default="X:\\dlas\\experiments\\train_diffusion_tts6_upsample.yml",
    )
    parser.add_argument(
        "-sr_diffusion_model_name",
        type=str,
        help="Name of the SR diffusion model in opt.",
        default="generator",
    )
    parser.add_argument(
        "-sr_diffusion_model_path",
        type=str,
        help="Path to saved model weights for the SR diffuser",
        default="X:\\dlas\\experiments\\train_diffusion_tts6_upsample_continued\\models\\53500_generator_ema.pth",
    )
    parser.add_argument(
        "-cond", type=str, help="Type of conditioning voice", default="plants"
    )
    parser.add_argument(
        "-diffusion_steps",
        type=int,
        help="Number of diffusion steps to perform to create the generate. Lower steps reduces quality, but >40 is generally pretty good.",
        default=100,
    )
    parser.add_argument(
        "-output_path",
        ...
        default="../results/use_diffuse_tts",
    )
    ...
    torchaudio.save(
        os.path.join(args.output_path, "cond_base.wav"), cond.cpu(), base_sample_rate
    )
    torchaudio.save(
        os.path.join(args.output_path, "cond_sr.wav"), sr_cond.cpu(), sr_sample_rate
    )

    print("Generating codes for text..")
    codegen = load_model_from_config(
        args.opt_code_gen,
        args.code_gen_model_name,
        also_load_savepoint=False,
        load_path=args.code_gen_model_path,
        device="cuda",
    ).eval()
    ...
    aligned_codes_compression_factor = base_sample_rate * 221 // 11025
    sr_diffusion = load_model_from_config(
        args.sr_opt,
        args.sr_diffusion_model_name,
        also_load_savepoint=False,
        load_path=args.sr_diffusion_model_path,
        device="cpu",
    ).eval()
    ...
    with torch.no_grad():
        for p, code in enumerate([codes]):
            ...
            output_base = diffuser.p_sample_loop(
                diffusion,
                output_shape,
                noise=torch.zeros(output_shape, device=args.device),
                model_kwargs={
                    "tokens": aligned_codes,
                    "conditioning_input": cond.unsqueeze(0),
                },
            )
            diffusion = diffusion.cpu()
            torchaudio.save(
                os.path.join(args.output_path, f"{p}_output_mean_base.wav"),
                output_base.cpu().squeeze(0),
                base_sample_rate,
            )
            ...
            output_shape = (
                1,
                1,
                output_base.shape[-1] * (sr_sample_rate // base_sample_rate),
            )
            sr_diffusion = sr_diffusion.cuda()
            output = sr_diffuser.p_sample_loop(
                sr_diffusion,
                output_shape,
                noise=torch.zeros(output_shape, device=args.device),
                model_kwargs={
                    "tokens": torch.zeros_like(aligned_codes),
            ...
            torchaudio.save(
                os.path.join(args.output_path, f"{p}_output_mean_sr.wav"),
