...
def process_file(
    file, base_path, output_path, progress_file, duration_per_clip, sampling_rate=22050
):
    ...
    outdir = (
        os.path.join(output_path, f"{os.path.relpath(file, base_path)[:-4]}")
        .replace(".", "")
        .strip()
    )
    ...
    for i, spl in enumerate(splits):
        ...
        torchaudio.save(
            f"{outdir}/{i:05d}.wav", spl.unsqueeze(0), sampling_rate, encoding="PCM_S"
        )
    report_progress(progress_file, file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-progress_file",
        type=str,
        help="Place to store all files that have already been processed",
    ...
    parser.add_argument(
        "-duration", type=int, help="Duration per clip in seconds", default=30
    )
    ...
    print(
        f"Found {len(files)} files to process. Total processing is {100*(orig_len-len(files))/orig_len}% complete."
    )
