...
def process_folder(
    folder, base_path, output_path, progress_file, max_duration, sampling_rate=22050
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_duration", type=int, help="Duration per clip in seconds", default=120
    )
    ...
    print(
        f"Found {len(folders)} files to process. Total processing is {100 * (orig_len - len(folders)) / orig_len}% complete."
    )
