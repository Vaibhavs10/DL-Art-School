...
def forward_pass(model, denoiser, data, output_dir, opt, b):
    ...
    for i in range(pred_waveforms.shape[0]):
        ...
        pred_mel = (
            (pred_mel - pred_mel.mean()) / max(abs(pred_mel.min()), pred_mel.max())
        ).unsqueeze(1)
        torchvision.utils.save_image(
            pred_mel, osp.join(output_dir, f"{b}_{i}_pred_mel.png")
        )
        audio = pred_waveforms[i][0].cpu().numpy()
        wavfile.write(osp.join(output_dir, f"{b}_{i}.wav"), 22050, audio)

        if gt:
            gt_mel = model.eval_state[opt["eval"]["ground_truth_mel"]][0][i].unsqueeze(
                0
            )
            gt_mel = (
                (gt_mel - gt_mel.mean()) / max(abs(gt_mel.min()), gt_mel.max())
            ).unsqueeze(1)
            torchvision.utils.save_image(
...
if __name__ == "__main__":
    ...
    for phase, dataset_opt in sorted(opt["datasets"].items()):
        ...
        logger.info(
            "Number of test texts in [{:s}]: {:d}".format(
    ...
    denoiser = Denoiser(
        model.networks["waveglow"].module
