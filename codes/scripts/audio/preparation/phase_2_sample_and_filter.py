...
class AudioFolderDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if audio_norm.shape[-1] > self.pad_to:
            print(
                f"Warning - {path} has a longer audio clip than is allowed: {audio_norm.shape[-1]}; allowed: {self.pad_to}. "
        ...
        return {"clip": audio_norm, "samples": orig_length, "path": path}
...
def process_folder(folder, output_path, base_path, progress_file, max_files):
    classifier = (
        load_model_from_config(
            args.classifier_model_opt, model_name="classifier", also_load_savepoint=True
        )
        .cuda()
        .eval()
    )
        ...
        for batch in tqdm(dataloader):
            try:
                ...
                def get_spec_mags(clip):
                    stft = torch.stft(
                        clip, n_fft=22000, hop_length=1024, return_complex=True
                    )
                    stft = stft[0, -2000:, :]
                    return (stft.real**2 + stft.imag**2).sqrt()

                no_hifreq_data = get_spec_mags(clips).mean(dim=1) < 0.01
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path",
        type=str,
        help="Path to search for split files (should be the direct output of phase 1)",
        default="Y:\\clips\\red_rising_split",
    )
    parser.add_argument(
        "--progress_file",
        type=str,
        help="Place to store all folders that have already been processed",
        default="Y:\\clips\\red_rising_filtered\\already_processed.txt",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        help="Path where sampled&filtered files are sent",
        default="Y:\\clips\\red_rising_filtered",
    )
    parser.add_argument(
        "--num_threads",
        type=int,
        help="Number of concurrent workers processing files.",
        default=6,
    )
    parser.add_argument(
        "--max_samples_per_folder",
        type=int,
        help="Maximum number of clips that can be extracted from each folder.",
        default=999999,
    )
    parser.add_argument(
        "--classifier_model_opt",
        type=str,
        help="Train/test options file that configures the model used to classify the audio clips.",
            ...
            tqdm(
                pool.imap(
                    functools.partial(
                        ...
                        max_files=args.max_samples_per_folder,
                    ),
                    all_split_files,
                ),
                total=len(all_split_files),
            )
        )
