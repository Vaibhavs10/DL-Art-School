...
def main():
    ...
    opt["exclusions"] = [
        [],
        [],
    ]  # image names matching these terms wont be included in the processing.
    opt["thres_sz"] = 129  # size threshold
    opt["resize_final_img"] = [1, 0.5]
    ...
    opt[
        "input_image_max_size_before_being_halved"
    ] = 5500  # As described, images larger than this dimensional size will be halved before anything else is done.
...
class LmdbWriter:
    def __init__(
        self, lmdb_path, max_mem_size=30 * 1024 * 1024 * 1024, write_freq=5000
    ):
        self.db = lmdb.open(
            lmdb_path,
            subdir=True,
            map_size=max_mem_size,
            readonly=False,
            meminit=False,
            map_async=True,
        )
    ...
    def write_tile_image(self, ref_id, tile_image):
        next_tile_id = (
            0 if ref_id not in self.tile_ids.keys() else self.tile_ids[ref_id]
        )
    ...
    def write_image(self, id, img, center_point):
        self.txn.put(
            ...
            pyarrow.serialize(img).to_buffer(),
            pyarrow.serialize(center_point).to_buffer(),
        )
        self.writes += 1
...
class FileWriter:
    def __init__(self, folder):
        ...
        self.ref_center_points = (
            {}
        )  # Maps ref_img basename to a dict of image IDs:center points
    ...
    def write_reference_image(self, ref_img, path):
        (
            ...
            _,
            _,
        ) = ref_img  # Encoded with a center point, which is irrelevant for the reference image.
...
class TiledDataset(data.Dataset):
        ...
        for x in h_space:
            for y in w_space:
                ...
                if mask is not None:
                    ...
                    crop_mask = mask[
                        mask_map(x) : mask_map(x + crop_sz),
                        mask_map(y) : mask_map(y + crop_sz),
                        :,
                    ]
                    if (
                        crop_mask.mean() < 255 / 2
                    ):  # If at least 50% of the image isn't made up of the type of pixels we want to process, ignore this tile.
                        continue
                # Center point needs to be resized by ref_resize_factor - since it is relative to the reference image.
                center_point = (
                    int((x + crop_sz // 2) // ref_resize_factor),
                    int((y + crop_sz // 2) // ref_resize_factor),
                )
                ...
                assert success
                results.append(
                    (buffer, center_point, int(crop_sz // ref_resize_factor))
                )
        return results
    ...
    def get(self, index, split_mode, left_img):
        ...
        mask = (
            cv2.imread(
                os.path.join(self.opt["mask_path"], os.path.basename(path) + ".png"),
                cv2.IMREAD_UNCHANGED,
            )
            if self.opt["use_masking"]
            else None
        )
        ...
        success, ref_buffer = cv2.imencode(
            ".jpg", ref_img, [cv2.IMWRITE_JPEG_QUALITY, self.opt["compression_level"]]
        )
        assert success
        results = [(ref_buffer, (-1, -1), (-1, -1))]

        for crop_sz, exclusions, resize_factor, step in zip(
            self.opt["crop_sz"],
            self.opt["exclusions"],
            self.opt["resize_final_img"],
            self.opt["step"],
        ):
            excluded = False
            ...
            results.extend(
                self.get_for_scale(
                    img, mask, crop_sz, step, resize_factor, ref_resize_factor
                )
            )
        return results, path
...
def extract_single(opt, writer):
    ...
    dataloader = data.DataLoader(
        dataset, num_workers=opt["n_thread"], collate_fn=identity
    )
    tq = tqdm(dataloader)
    i = 0
    for spl_imgs in tq:
