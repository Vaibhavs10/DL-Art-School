...
def main():
    ...
    if mode == "single":
        ...
        opt["image_minimum_size_threshold"] = int(
            1024 * full_multiplier
        )  # Minimum size of input image in height dim. Images under this size will not be processed.
        opt["resize_final_img"] = 0.5
        ...
        assert len(img_GT_list) == len(
            img_LR_list
        ), "different length of GT_folder and LR_folder."
        for path_GT, path_LR in zip(img_GT_list, img_LR_list):
            ...
            assert (
                w_GT / w_LR == scale_ratio
            ), "GT width [{:d}] is not {:d}X as LR weight [{:d}] for {:s}.".format(  # noqa: E501
        ...
        assert (
            crop_sz % scale_ratio == 0
        ), "crop size is not {:d}X multiplication.".format(scale_ratio)
        assert step % scale_ratio == 0, "step is not {:d}X multiplication.".format(
            scale_ratio
        )
        assert (
            thres_sz % scale_ratio == 0
        ), "thres_sz is not {:d}X multiplication.".format(scale_ratio)
        print("process GT...")
        ...
        assert len(data_util._get_paths_from_images(save_GT_folder)) == len(
            data_util._get_paths_from_images(save_LR_folder)
    ...
    for x in h_space:
        for y in w_space:
            ...
            if "resize_final_img" in opt.keys():
                ...
                if dsize is None:
                    dsize = (
                        int(crop_img.shape[0] * resize_factor),
                        int(crop_img.shape[1] * resize_factor),
                    )
            ...
            cv2.imwrite(
                osp.join(
                    opt["save_folder"],
                    img_name.replace(
                        ".jpg", "_l{:05d}_s{:03d}.jpg".format(left, index)
                    ),
                ),
                crop_img,
