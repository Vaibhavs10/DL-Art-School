...
def forward_pass(model, data, output_dir, jc, bc):
    ...
    # save images
    suffixes = [
        f"_blur_{int(bc*1000)}_{int(jc*1000)}",
...
if __name__ == "__main__":
    ...
    jpegs = list(
        numpy.arange(
            opt["min_jpeg_correction"],
            opt["max_jpeg_correction"],
            opt["jpeg_correction_step_size"],
        )
    )
    deblurs = list(
        numpy.arange(
            opt["min_blur_correction"],
            opt["max_blur_correction"],
            opt["blur_correction_step_size"],
        )
    )
