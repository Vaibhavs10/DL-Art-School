...
def forward_pass(model, data, output_dir, spacing, audio_mode):
    ...
    for suffix in suffixes:
        if audio_mode:
            ...
            wavfile.write(
                osp.join(output_dir, save_img_path), 11025, sr_img[0].cpu().numpy()
            )
        else:
    ...
    for i in range(10):
        if audio_mode:
            data = {
                ...
                "num_alt_clips": torch.tensor(
                    [refs.shape[1]], dtype=torch.int32, device="cuda"
                ),
                "GT_path": opt["image"],
            ...
            data = {
                ...
                "corruption_entropy": torch.tensor(
                    [correction_factors], device="cuda", dtype=torch.float
                ),
                "GT_path": opt["image"],
            }
