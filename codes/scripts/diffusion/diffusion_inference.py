...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--blur_correction",
        type=float,
        help="Blur correction factor; [0,1]",
        default=0.1,
    )
    parser.add_argument(
        "--jpeg_correction",
        type=float,
        help="Compression noise correction factor; [0,1]",
        default=0,
    )
    parser.add_argument(
        "--sr_factor",
        type=int,
        help="Multiplicative amount to super-resolve the image; [1,4]",
        default=2,
    )
    parser.add_argument(
        "--diffusion_steps",
        type=int,
        help="Number of diffusion steps. Lower is faster, higher makes higher quality images. >400 is unnecessary. [0,4000]",
        default=100,
    )
    parser.add_argument(
        "--output", type=str, help="Where to store output image", default="."
    )
    parser.add_argument(
        "--device",
        type=str,
        help="Device to perform inference on; cpu or cuda",
        default="cuda",
    )
    ...
    diffuser = SpacedDiffusion(
        use_timesteps=space_timesteps(4000, [args.diffusion_steps]),
    ...
    with torch.no_grad():
        output_shape = (
            1,
            3,
            lr_image.shape[-2] * args.sr_factor,
            lr_image.shape[-1] * args.sr_factor,
        )
        cfactor = torch.tensor(
            [[args.jpeg_correction, args.blur_correction]],
            device=args.device,
            dtype=torch.float,
        )
        hq = diffuser.p_sample_loop(
            model,
            output_shape,
            model_kwargs={"low_res": lr_image, "corruption_factor": cfactor},
        )
        hq = (hq + 1) / 2
