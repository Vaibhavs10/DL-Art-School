    ...
    for batch in tqdm(dataloader):
        ...
        for b in range(latent.shape[0]):
            ...
            if (id + 1) % 1000 == 0:
                ...
                torch.save(
                    latent_dict,
                    os.path.join(output_path, "latent_dict_%i.pth" % (dict_count,)),
                )
                latent_dict = {}
...
def _get_mins_from_latent_dictionary(
    latent, hq_img_repo, ld_file_name, batch_size, compare_fn
):
    _, c, h, w = latent.shape
...
def find_similar_latents(
    model, model_index=0, lat_patch_size=16, compare_fn=structural_euc_dist
):
    img = (
    ...
    for i, ind in enumerate(tqdm(min_ids)):
        u = np.unravel_index(
            ind.item(),
            (
                num_maps * total_latents // (lat_patch_mult**2),
                lat_patch_mult,
                lat_patch_mult,
            ),
        )
        ...
        img_out[
            :,
            :,
            io_loc_t : io_loc_t + lat_patch_size,
            io_loc_l : io_loc_l + lat_patch_size,
        ] = patch

        # Also save the image with a masked map
        mask = torch.full_like(img, fill_value=0.3)
        ...
        img_map_areas.append(
            '<area shape="rect" coords="%i,%i,%i,%i" href="%s">'
            % (
                io_loc_l,
                io_loc_t,
                io_loc_l + lat_patch_size,
                io_loc_t + lat_patch_size,
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        find_similar_latents(
            model, 1, 16, structural_euc_dist
