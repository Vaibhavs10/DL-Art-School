...
class VideoClipDataset(data.Dataset):
    ...
    def extract_n_frames(self, video_file, dest, time_seconds, n):
        ffmpeg_args = [
            ...
            "-y",
            "-ss",
            get_time_for_secs(time_seconds),
            "-i",
            video_file,
            "-vframes",
            str(n),
    ...
    def get(self, index):
        ...
        while start < vid_len:
            ...
            assert len(frames) == n
            img_runs.append(
                ([self.get_image_tensor(frame) for frame in frames], frames_out)
            )
    ...
    for run in runs:
        ...
        for i in range(1, len(run)):
            ...
            flow = torch.nn.functional.interpolate(
                flow, size=img.shape[2:], mode="bilinear"
            )
            if consolidated_flows is None:
                consolidated_flows = flow
            else:
                consolidated_flows = (
                    resampler(flow, -consolidated_flows) + consolidated_flows
                )
            img = img2
            dbg = resampler(dbg, flow)
        torchvision.utils.save_image(dbg, os.path.join(path, "debug.jpg"))
        consolidated_flows = torch.clamp(consolidated_flows / 255, -0.5, 0.5)
        b = run[-1].unsqueeze(0).cuda()
        _, _, h, w = a.shape
        direct_flows = torch.nn.functional.interpolate(
            torch.clamp(flownet(torch.cat([a, b], dim=1).float()) / 255, -0.5, 0.5),
            size=img.shape[2:],
            mode="bilinear",
        )
        ...
        torchvision.utils.save_image(
            torch.cat(
                [consolidated_flows + 0.5, torch.zeros((1, 1, h, w), device="cuda")],
                dim=1,
            ),
            os.path.join(path, "consolidated_flow.png"),
        )

        # For debugging
        torchvision.utils.save_image(
            resampler(a, consolidated_flows * 255), os.path.join(path, "b_flowed.jpg")
        )
        torchvision.utils.save_image(
            resampler(b, -consolidated_flows * 255), os.path.join(path, "a_flowed.jpg")
        )
        torchvision.utils.save_image(
            resampler(b, direct_flows * 255),
            os.path.join(path, "a_flowed_nonconsolidated.jpg"),
        )
        torchvision.utils.save_image(
            torch.cat(
                [direct_flows + 0.5, torch.zeros((1, 1, h, w), device="cuda")], dim=1
            ),
            os.path.join(path, "direct_flow.png"),
        )
