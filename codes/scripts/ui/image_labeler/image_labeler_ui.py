...
def update_mode_label():
    ...
    mode_label.config(
        text="Current mode: %s; Saved images: %i"
...
def next_batch():
    ...
    ) * 1.0  # This results in a tensor of 0's where tensors are outside of the confidence bound, 1's where inside.
    ...
    while (
        ...
        for p in permutation:
            ...
            mask = (valid_res[cur_img, mode, :, :].clone() * 0.25 + 0.25).unsqueeze(0)
            mask[:, t, l] = 1.0
            ...
            primary_label.config(
                text="%s (p=%f)" % (labeler.str_labels[lblis[0].item()], probs[0])
            )
            secondary_label.config(
                text="%s (p=%f)" % (labeler.str_labels[lblis[1].item()], probs[1])
            )
...
if __name__ == "__main__":
    ...
    help = tk.Label(
        window,
        text="Next: ctrl+f, Mode: ctrl+x, Undo: ctrl+z, Save: ctrl+s",
        anchor="w",
    )
    ...
    mode_label = tk.Label(
        window,
        text="Current mode: %s; Saved images: %i" % (labeler.str_labels[mode], 0),
        anchor="w",
    )
