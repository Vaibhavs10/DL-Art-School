...
def image_2_tensor(impath, max_size=None):
    ...
    if max_size is not None:
        ...
        new_size = (
            int(math.ceil(img.width * factor)),
            int(math.ceil(img.height * factor)),
        )
...
def fetch_latents_for_image(gen, img, scale, lr_infer=interpolate_lr):
    z, _, _ = gen(
        gt=img, lr=lr_infer(img, scale), epses=[], reverse=False, add_gt_noise=False
    )
    return z
...
def linear_interpolation(latents1, latents2, proportion):
    return [
        l1 * proportion + l2 * (1 - proportion) for l1, l2 in zip(latents1, latents2)
    ]
...
def slerp(latents1, latents2, proportion):
    ...
    for low, high in zip(latents1, latents2):
        ...
        so = torch.sin(omega)
        res.append(
            (torch.sin((1.0 - proportion) * omega) / so).unsqueeze(1) * low
            + (torch.sin(proportion * omega) / so).unsqueeze(1) * high
        )
    return res


def create_interpolation_video(
    gen, lq, output_file, latents1, latents2, steps=10, prefix=""
):
    ...
    for i in range(steps):
        ...
        hr, _, _ = gen(lr=lq, z=lats[0], reverse=True, epses=lats, add_gt_noise=False)
        torchvision.transforms.ToPILImage()(hr.squeeze(0).cpu())
        torchvision.utils.save_image(
            hr.cpu(), os.path.join(output_file, "%s_%i.png" % (prefix, i))
        )
...
if __name__ == "__main__":
    ...
    max_ref_datatypes = (
        30  # Only picks this many images from the above data types to sample from.
    )
    ...
    with torch.no_grad():
        ...
        if mode == "latent_transfer":
            ...
            corruptor = ImageCorruptor(
                {"fixed_corruptions": ["jpeg-medium", "gaussian_blur_3"]}
            )
            ...
            dt_latents = [
                fetch_latents_for_image(gen, i, scale, corrupt_and_downsample)
                for i in dt_transfers
            ]
        ...
        for im_it, img_file in enumerate(tqdm(img_files)):
            ...
            if mode == "restore":
                ...
                for slat in dt_latents:
                    ...
                    dts.append(
                        [
                            extract_center_latent(sl, l) * temperature
                            for l, sl in zip(latents, slat)
                        ]
                    )
                latents = dts
            ...
            for j in range(len(lats)):
                ...
                torchvision.utils.save_image(
                    resample_img, os.path.join(path, "orig_%i.jpg" % (im_it))
                )
                create_interpolation_video(
                    gen,
                    F.interpolate(resample_img, scale_factor=1 / scale, mode="area"),
                    path,
                    [torch.zeros_like(l) for l in lats[j]],
                    lats[j],
                    prefix=mode,
                )
