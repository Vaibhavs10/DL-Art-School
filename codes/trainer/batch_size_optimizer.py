...
class GradientDirectionOptimizer(BatchSizeOptimizer):
    ...
    def focus(self, model):
        if (
            not hasattr(model, "_gradient_direction_optimizer_finished")
            or model._gradient_direction_optimizer_finished
        ):
            all_params = list(
                filter(
                    lambda t: ".weight" in t[0]
                    and not hasattr(t[1].requires_grad, "DO_NOT_TRAIN"),
    ...
    def should_step(self, it):
        ...
        if model._gradient_direction_optimizer_prior_grads is not None:
            cur_dir = [
                ...
                for lgrad, cgrad in zip(
                    model._gradient_direction_optimizer_prior_grads, cur_grads
                )
            ]
            delta_dir = [
                (cdir - ldir)
                for cdir, ldir in zip(
                    cur_dir, model._gradient_direction_optimizer_prior_directions
                )
            ]
            ...
            if (
                all_finished
                or model._gradient_direction_optimizer_step >= self.max_full_batches
            ):
    ...
    def record_number_steps(self, steps):
        ...
        self.last_number_iterations_i = (
            self.last_number_iterations_i + 1
        ) % self.last_number_iterations.shape[0]
        self.steps_taken += 1
    ...
    def get_statistics(self):
        ...
        if self.last_number_iterations_filled:
            res[
                "batch_size_opt_avg_iterations_per_step"
            ] = self.last_number_iterations.mean().item()
        else:
            res["batch_size_opt_avg_iterations_per_step"] = (
                self.last_number_iterations[: self.last_number_iterations_i]
                .mean()
                .item()
            )
        return res
