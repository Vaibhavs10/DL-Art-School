...
from data.images.image_pair_with_corresponding_points_dataset import (
...
class SinglePointPairContrastiveEval(evaluator.Evaluator):
    ...
    def get_l2_score(self, dl, dev):
        ...
        for i, data in tqdm(enumerate(dl)):
            latent1 = self.model(
                img=data["img1"].to(dev),
                pos=torch.stack(data["coords1"], dim=1).to(dev),
            )
            latent2 = self.model(
                img=data["img2"].to(dev),
                pos=torch.stack(data["coords2"], dim=1).to(dev),
            )
    ...
    def perform_eval(self):
        ...
        self.model.train()
        return {
            "val_similar_l2": similars.item(),
            "val_dissimilar_l2": dissimilars.item(),
            "val_diff": diff,
        }
