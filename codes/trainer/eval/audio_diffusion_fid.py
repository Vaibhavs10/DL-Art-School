...
class AudioDiffusionFid(evaluator.Evaluator):
    ...
    def perform_original_diffusion_vocoder(self, audio, codes, text, sample_rate=11025):
        ...
        real_resampled = torchaudio.functional.resample(
            real_resampled.squeeze(0), sample_rate, 5500
        ).unsqueeze(0)
        gen = torchaudio.functional.resample(
            gen.squeeze(0), sample_rate, 5500
        ).unsqueeze(0)
    ...
    def perform_diffusion_vocoder(self, audio, codes, text, sample_rate=5500):
        ...
        gen = self.diffuser.p_sample_loop(
            ...
            model_kwargs={
                ...
                "unaligned_input": torch.tensor(
                    text_codes, device=audio.device
                ).unsqueeze(0),
            },
        )
        return gen, real_resampled, sample_rate
    ...
    def tts9_get_autoregressive_codes(self, mel, text):
        mel_codes = convert_mel_to_codes(self.local_modules["dvae"], mel)
        text_codes = (
            torch.LongTensor(self.bpe_tokenizer.encode(text))
            .unsqueeze(0)
            .to(mel.device)
        )
    ...
    def perform_diffusion_tts9_mel_from_codes(self, audio, codes, text):
        ...
        gen_mel = self.diffuser.p_sample_loop(
            ...
            output_shape,
            model_kwargs={
                "aligned_conditioning": mel_codes,
    ...
    def perform_diffusion_tfd_ar_prior(self, audio, codes, text):
        ...
        gen_mel = self.diffuser.p_sample_loop(
            self.model, umel.shape, model_kwargs={"codes": auto_latents}
        )
    ...
    def perform_eval(self):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(list(range(0, len(self.data), self.skip))):
                ...
                intelligibility_losses.append(
                    self.intelligibility_loss(w2v, sample, ref, sample_rate, text)
                )

                torchvision.utils.save_image(
                    (gen_mel.unsqueeze(1) + 1) / 2,
                    os.path.join(save_path, f'{self.env["rank"]}_{i}_mel.png'),
                )
                torchvision.utils.save_image(
                    (ref_mel.unsqueeze(1) + 1) / 2,
                    os.path.join(save_path, f'{self.env["rank"]}_{i}_mel_target.png'),
                )
                torchaudio.save(
