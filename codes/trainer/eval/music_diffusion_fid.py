...
class MusicDiffusionFid(evaluator.Evaluator):
    ...
    def __init__(self, model, opt_eval, env):
        ...
        self.clip = opt_get(
            opt_eval, ["clip_audio"], True
        )  # Recommend setting true for more efficient eval passes.
        ...
        self.projector.load_state_dict(
            torch.load(
                "../experiments/music_eval_projector.pth",
        ...
        if mode == "spec_decode":
            ...
            self.kmeans_inj = KmeansQuantizerInjector(
                {
                    "centroids": "../experiments/music_k_means_centroids.pth",
                    "in": "in",
                    "out": "out",
                },
                {},
    ...
    def perform_diffusion_spec_decode(self, audio, sample_rate=22050):
        ...
        gen = self.diffuser.p_sample_loop(
            self.model, output_shape, model_kwargs={"codes": mel}
        )
    ...
    def perform_diffusion_from_codes_quant(self, audio, sample_rate=22050):
        ...
        sampler = (
            self.diffuser.ddim_sample_loop if self.ddim else self.diffuser.p_sample_loop
        )
        gen_mel = sampler(
            self.model, mel_norm.shape, model_kwargs={"truth_mel": mel_norm}
        )
        ...
        self.spec_decoder = self.spec_decoder.to(audio.device)
        sampler = (
            self.spectral_diffuser.ddim_sample_loop
            if self.ddim
            else self.spectral_diffuser.p_sample_loop
        )
        gen_wav = sampler(
    ...
    def perform_reconstruction_from_cheater_gen(self, audio, sample_rate=22050):
        ...
        real_wav = self.spectral_diffuser.ddim_sample_loop(
            self.spec_decoder, output_shape, model_kwargs={"codes": mel}
        )
        real_wav = pixel_shuffle_1d(real_wav, 16)
    ...
    def perform_diffusion_from_codes_ar_prior(self, audio, sample_rate=22050):
        ...
        ar_latent = self.local_modules["ar_prior"].to(audio.device)(
            cheater_codes, cheater, return_latent=True
        )
    ...
    def perform_eval(self):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(list(range(0, len(self.data), self.skip))):
                ...
                torchaudio.save(
                    os.path.join(save_path, f"{self.env['rank']}_{i}_gen.wav"),
                    sample.squeeze(0).cpu(),
                    sample_rate,
                )
                torchaudio.save(
                    os.path.join(save_path, f"{self.env['rank']}_{i}_real.wav"),
                    ref.cpu(),
                    sample_rate,
                )
                torchvision.utils.save_image(
                    (sample_mel.unsqueeze(1) + 1) / 2,
                    os.path.join(save_path, f"{self.env['rank']}_{i}_gen_mel.png"),
                )
                torchvision.utils.save_image(
                    (ref_mel.unsqueeze(1) + 1) / 2,
                    os.path.join(save_path, f"{self.env['rank']}_{i}_real_mel.png"),
                )
