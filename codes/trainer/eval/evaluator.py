    ...
    for mod in module_iter:
        if mod.ispkg:
            ...
            if "eval_wer" in mod.name:
                continue  # TODO: this causes an import error for PyCtcDecode. get rid of this if there's a need to use that evaluator.
...
class CreateEvaluatorError(Exception):
    def __init__(self, name, available):
        super().__init__(
            f"Could not find the specified evaluator name: {name}.  Available evaluators:"
            f"{available}"
        )
