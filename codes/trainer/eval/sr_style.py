...
class SrStyleTransferEvaluator(evaluator.Evaluator):
    def __init__(self, model, opt_eval, env):
        ...
        self.gen_output_index = (
            opt_eval["gen_index"] if "gen_index" in opt_eval.keys() else 0
        )
        self.dataset = Stylegan2Dataset(
            {
                "path": self.fid_real_samples,
    ...
    def perform_eval(self):
        ...
        for batch in self.sampler:
            ...
            for b in range(self.batch_sz):
                ...
                torchvision.utils.save_image(
                    batch_hq[b], osp.join(fid_real_path, "%i_.png" % (counter))
                )
                counter += 1

        return {
            "fid": fid_score.calculate_fid_given_paths(
                [fid_real_path, fid_fake_path], self.batch_sz, True, 2048
            )
        }
