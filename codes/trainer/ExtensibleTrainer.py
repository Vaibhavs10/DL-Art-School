...
class ExtensibleTrainer(BaseModel):
    def __init__(self, opt, cached_networks={}):
        ...
        for name, net in opt["networks"].items():
            ...
            if net["type"] == "generator":
                if new_net is None:
                    new_net = networks.create_model(opt, net, self.netsG).to(
                        self.device
                    )
                self.netsG[name] = new_net
            elif net["type"] == "discriminator":
                if new_net is None:
                    new_net = networks.create_model(opt, net, self.netsD).to(
                        self.device
                    )
            ...
            if has_any_trainable_params and opt["dist"]:
                if opt["dist_backend"] == "apex":
                    ...
                    dnet = DistributedDataParallel(
                        ...
                        find_unused_parameters=opt_get(
                            opt, ["ddp_find_unused_parameters"], False
                        ),
                    )
    ...
    def feed_data(self, data, step, need_GT=True, perform_micro_batching=True):
        ...
        for k, v in data.items():
            ...
            if isinstance(v, torch.Tensor):
                self.dstate[k] = [
                    t.to(self.device)
                    for t in torch.chunk(v, chunks=batch_factor, dim=0)
                ]
        ...
        for net in self.networks.values():
            if hasattr(net.module, "update_for_step"):
                net.module.update_for_step(
                    it, os.path.join(self.opt["path"]["models"], "..")
                )
        ...
        for step_num, step in enumerate(self.steps):
            ...
            if train_step:
                ...
                for name, net in self.networks.items():
                    ...
                    if (
                        "after" in self.opt["networks"][name].keys()
                        and it < self.opt["networks"][name]["after"]
                    ):
                        net_enabled = False
                    for p in net.parameters():
                        do_not_train_flag = hasattr(p, "DO_NOT_TRAIN") or (
                            hasattr(p, "DO_NOT_TRAIN_UNTIL")
                            and it < p.DO_NOT_TRAIN_UNTIL
                        )
                        if (
                            p.dtype != torch.int64
                            and p.dtype != torch.bool
                            and not do_not_train_flag
                        ):
            ...
            if train_step and optimize and self.batch_size_optimizer.should_step(it):
                ...
                for opt in step.optimizers:
                    ...
                    if (
                        ...
                        and step.scaler._per_optimizer_states[id(opt)]["stage"]
                        is not OptState.UNSCALED
                    ):
                ...
                if self.auto_scale_grads:
                    ...
                    for net in self.networks.values():
                        for mod in net.modules():
                            ...
                            elif isinstance(mod, nn.Conv2d) or isinstance(
                                mod, nn.Conv3d
                            ):
                ...
                if return_grad_norms and train_step:
                    for name in nets_to_train:
                        ...
                        if hasattr(model.module, "get_grad_norm_parameter_groups"):
                            pgroups = {
                                f"{name}_{k}": v
                                for k, v in model.module.get_grad_norm_parameter_groups().items()
                            }
                        else:
                            pgroups = {
                                f"{name}_all_parameters": list(model.parameters())
                            }
        ...
        if (
            ...
            for v in self.opt["logger"]["visuals"]:
                ...
                for i, dbgv in enumerate(state[v]):
                    if (
                        "recurrent_visual_indices" in self.opt["logger"].keys()
                        and len(dbgv.shape) == 5
                    ):
                        for rvi in self.opt["logger"]["recurrent_visual_indices"]:
                            rdbgv = fix_image(dbgv[:, rvi])
                            os.makedirs(
                                os.path.join(sample_save_path, v), exist_ok=True
                            )
                            utils.save_image(
                                rdbgv.float(),
                                os.path.join(
    ...
    def limit_number_of_checkpoints_and_states(
        self,
        network_name: Union[str, Literal["ddpm", "gpt"]],
        models_number: int = 2,
        state_number: int = 2,
    ) -> None:
        if network_name not in ["ddpm", "gpt"]:
            raise ValueError(
                "Expected network name (opt['networks'][*]) to be 'ddpm' or 'gpt', but got %s"
                % network_name,
            )
        ...
        if not self.opt["logger"]["disable_state_saving"] and state_number > 0:
            files_state = sorted(
                states_path.glob("*.state"), reverse=True, key=lambda p: int(p.stem)
            )

        files_to_keep = (
            files_pth[: models_number - 1]
