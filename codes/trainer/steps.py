...
class ConfigurableStep(Module):
    def __init__(self, opt_step, env):
        ...
        self.scaler = GradScaler(
            enabled=self.opt["fp16"]
            or opt_get(self.opt, ["grad_scaler_enabled"], False)
        )
        ...
        if "injectors" in self.step_opt.keys():
            ...
            for inj_name, injector in self.step_opt["injectors"].items():
                assert (
                    inj_name not in injector_names
        ...
        if "losses" in self.step_opt.keys():
            for loss_name, loss in self.step_opt["losses"].items():
                assert (
                    loss_name not in self.weights.keys()
                )  # Repeated names are always an error case.
    ...
    def get_network_for_name(self, name):
        return (
            self.env["generators"][name]
            if name in self.env["generators"].keys()
    ...
    def define_optimizers(self):
        ...
        for net_name, net, opt_config in zip(training, nets, opt_configs):
            ...
            if (
                "optimizer" not in self.step_opt.keys()
                or self.step_opt["optimizer"] == "adamw"
            ):
                groups = [
                    {
                        "params": params_weights,
                        "weight_decay": opt_get(opt_config, ["weight_decay"], 0),
                    },
                ...
                opt = mbnb.optim.AdamW(
                    ...
                    betas=(
                        opt_get(opt_config, ["beta1"], 0.9),
                        opt_get(opt_config, ["beta2"], 0.999),
                    ),
                )
                ...
                opt._group_names = sorted(list(all_param_names))
            opt._config = (
                opt_config  # This is a bit seedy, but we will need these configs later.
            )
    ...
    def do_forward_backward(
        ...
        loss_accumulator = (
            self.loss_accumulator if loss_accumulator is None else loss_accumulator
        )
        ...
        if len(self.losses) > 0:
            ...
            for loss_name, loss in self.losses.items():
                ...
                if loss.is_stateful():
                    l, lstate = loss(
                        self.get_network_for_name(self.step_opt["training"]),
                        local_state,
                    )
            ...
            if train and isinstance(total_loss, torch.Tensor) and total_loss.isfinite():
                loss_accumulator.add_loss(
                    "%s_total" % (self.get_training_network_name(),), total_loss
                )
