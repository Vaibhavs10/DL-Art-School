...
def extract_params_from_state(
    params: object, state: object, root: object = True
) -> object:
    ...
    def forward(self, _, state):
        ...
        if self.opt["rescale"]:
            labels = F.interpolate(
                labels.type(torch.float), size=logits.shape[2:], mode="nearest"
            ).type(torch.long)
...
class PixLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.report_metrics = (
            opt["report_metrics"] if "report_metrics" in opt.keys() else False
        )
    ...
    def forward(self, _, state):
        ...
        if self.report_metrics:
            self.metrics.append(
                ("real_pix_mean_histogram", torch.mean(real, dim=[1, 2, 3]).detach())
            )
            self.metrics.append(
                ("fake_pix_mean_histogram", torch.mean(fake, dim=[1, 2, 3]).detach())
            )
...
class SrPixLoss(ConfigurableLoss):
    ...
    def forward(self, _, state):
        ...
        no_hf = F.interpolate(
            F.interpolate(real, scale_factor=1 / self.scale, mode="area"),
...
class DirectLoss(ConfigurableLoss):
    ...
    def forward(self, _, state):
        ...
        if self.anneal > 0:
            loss = loss * (
                1 - (self.anneal - min(self.env["step"], self.anneal)) / self.anneal
            )
        return loss
...
class FeatureLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.netF = trainer.networks.define_F(
            ...
            load_path=opt["load_path"] if "load_path" in opt.keys() else None,
        ).to(self.env["device"])
        if not env["opt"]["dist"]:
            self.netF = torch.nn.parallel.DataParallel(
                self.netF, device_ids=env["opt"]["gpu_ids"]
            )
    ...
    def forward(self, _, state):
        ...
        if self.opt["criterion"] == "cosine":
            return self.criterion(
                logits_fake.float(),
                logits_real.float(),
                torch.ones(1, device=logits_fake.device),
            )
        else:
...
class InterpretedFeatureLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.netF_real = trainer.networks.define_F(which_model=opt["which_model_F"]).to(
            self.env["device"]
        )
        self.netF_gen = trainer.networks.define_F(
            which_model=opt["which_model_F"], load_path=opt["load_path"]
        ).to(self.env["device"])
        if not env["opt"]["dist"]:
    ...
    def forward(self, _, state):
        ...
        with autocast(enabled=self.env["opt"]["fp16"]):
            if self.opt["gan_type"] in ["gan", "pixgan", "pixgan_fea"]:
                ...
                loss = (
                    self.criterion(pred_d_real - torch.mean(pred_g_fake), False)
...
class DiscriminatorGanLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.gradient_penalty = (
            opt["gradient_penalty"] if "gradient_penalty" in opt.keys() else False
        )
        if self.min_loss != 0:
            assert not self.env[
                "dist"
    ...
    def forward(self, net, state):
        ...
        if self.opt["gan_type"] in ["gan", "pixgan"]:
            ...
            self.metrics.append(("d_fake_diff", torch.mean(d_fake_diff)))
            loss = self.criterion(d_real - torch.mean(d_fake), True) + self.criterion(
                d_fake_diff, False
            )
        else:
            raise NotImplementedError
        ...
        if self.gradient_penalty:
            ...
            assert (
                len(real) == 1
            )  # Grad penalty doesn't currently support multi-input discriminators.
            gp, gp_structure = gradient_penalty(
                real[0], d_real, return_structured_grads=True
            )
            ...
            gpimg = (
                (
                    gp_structure
                    / (torch.std(gp_structure, dim=(-1, -2), keepdim=True) * 2)
                )
                - torch.mean(gp_structure, dim=(-1, -2), keepdim=True)
                + 0.5
            )
            new_state[
                "%s_%s_gp_structure_img" % (self.opt["fake"], self.opt["real"])
            ] = gpimg
...
class GeometricSimilarityGeneratorLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.criterion = get_basic_criterion_for_name(opt["criterion"], env["device"])
        self.gen_input_for_alteration = (
            opt["input_alteration_index"]
            if "input_alteration_index" in opt.keys()
            else 0
        )
        self.gen_output_to_use = (
            opt["generator_output_index"]
            if "generator_output_index" in opt.keys()
            else None
        )
    ...
    def random_alteration(self):
        return random.choice(
            [
                (
                    functools.partial(torch.flip, dims=(2,)),
                    functools.partial(torch.flip, dims=(2,)),
                ),
                (
                    functools.partial(torch.flip, dims=(3,)),
                    functools.partial(torch.flip, dims=(3,)),
                ),
                (
                    functools.partial(torch.rot90, k=1, dims=[2, 3]),
                    functools.partial(torch.rot90, k=3, dims=[2, 3]),
                ),
                (
                    functools.partial(torch.rot90, k=2, dims=[2, 3]),
                    functools.partial(torch.rot90, k=2, dims=[2, 3]),
                ),
                (
                    functools.partial(torch.rot90, k=3, dims=[2, 3]),
    ...
    def forward(self, net, state):
        net = self.env["generators"][
            self.generator
        ]  # Get the network from an explicit parameter.
        ...
        if self.opt["criterion"] == "cosine":
            return self.criterion(
                state[self.opt["real"]],
                upsampled_altered,
                torch.ones(1, device=upsampled_altered.device),
            )
        else:
            return self.criterion(
                state[self.opt["real"]].float(), upsampled_altered.float()
            )
...
class TranslationInvarianceLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.overlap = opt[
            "overlap"
        ]  # For maximum overlap, can be calculated as 2*patch_size-image_size
    ...
    def forward(self, net, state):
        ...
        fake[self.gen_input_for_alteration] = "%s_%s" % (
            fake[self.gen_input_for_alteration],
            trans_name,
        )
        ...
        real_shared_output = gen_output[
            :,
            :,
            border_sz : border_sz + self.overlap,
            border_sz : border_sz + self.overlap,
        ]
        ...
        if self.opt["criterion"] == "cosine":
            return self.criterion(
                fake_shared_output,
                real_shared_output,
                torch.ones(1, device=real_shared_output.device),
            )
        else:
            return self.criterion(
                fake_shared_output.float(), real_shared_output.float()
            )
...
class RecursiveInvarianceLoss(ConfigurableLoss):
    ...
    def forward(self, net, state):
        ...
        if self.opt["criterion"] == "cosine":
            return self.criterion(
                compare_real, compare_fake, torch.ones(1, device=compare_real.device)
            )
        else:
...
class RecurrentLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.recurrent_weights = (
            opt["recurrent_weights"] if "recurrent_weights" in opt.keys() else 1
        )
