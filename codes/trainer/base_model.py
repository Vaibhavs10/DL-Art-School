...
class BaseModel:
    def __init__(self, opt):
        ...
        self.device = (
            torch.cuda.current_device() if opt["gpu_ids"] else torch.device("cpu")
        )
    ...
    def save_network(self, network, network_label, iter_label):
        ...
        if "alt_path" in self.opt["path"].keys():
            torch.save(
                state_dict, os.path.join(self.opt["path"]["alt_path"], save_filename)
            )
        if self.opt["colab_mode"]:
            utils.util.copy_files_to_server(
                self.opt["ssh_server"],
    ...
    def load_network(self, load_path, network, strict=True, pretrain_base_path=None):
        ...
        if hasattr(network, "module"):
            network = network.module
        load_net = torch.load(
            load_path, map_location=utils.util.map_cuda_to_correct_device
        )
    ...
    def save_training_state(self, state):
        ...
        save_filename = "{}.state".format(
            utils.util.opt_get(state, ["iter"], "no_step_provided")
        )
        ...
        if "alt_path" in self.opt["path"].keys():
            torch.save(
                state, os.path.join(self.opt["path"]["alt_path"], "latest.state")
            )
        if self.opt["colab_mode"]:
            utils.util.copy_files_to_server(
                self.opt["ssh_server"],
