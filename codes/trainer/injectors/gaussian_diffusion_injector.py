...
class GaussianDiffusionInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.deterministic_sampler = DeterministicSampler(
            self.diffusion,
            opt_get(opt, ["deterministic_sampler_expected_batch_size"], 2048),
            env,
        )
        ...
        if "channel_restriction_low" in opt.keys():
            self.channel_balancing_fn = functools.partial(
                channel_restriction,
                low=opt["channel_restriction_low"],
                high=opt["channel_restriction_high"],
            )
            k += 1
    ...
    def extra_metrics(self):
        ...
        uql = (
            (self.latest_mse_by_batch * uqt).sum() / uqt.sum() if uqt.sum() != 0 else 0
        )
        muqt = (self.latest_timesteps > self.num_timesteps / 2) * (
            self.latest_timesteps < self.num_timesteps * 3 / 4
        )
        muql = (
            (self.latest_mse_by_batch * muqt).sum() / muqt.sum()
            if muqt.sum() != 0
            else 0
        )
        d = {
        ...
        if hasattr(self, "schedule_sampler") and isinstance(
            self.schedule_sampler, LossSecondMomentResampler
        ):
    ...
    def forward(self, state):
        ...
        hq = state[self.input]
        assert (
            hq.max() < 1.000001 or hq.min() > -1.00001
        ), f"Attempting to train gaussian diffusion on un-normalized inputs. This won't work, silly! {hq.min()} {hq.max()}"

        with autocast(enabled=self.env["opt"]["fp16"]):
            if not gen.training or (
                self.deterministic_timesteps_every != 0
                and self.env["step"] % self.deterministic_timesteps_every == 0
            ):
            ...
            if self.causal_mode:
                ...
                diffusion_outputs = self.diffusion.causal_training_losses(
                    gen,
                    hq,
                    t,
                ...
                diffusion_outputs = self.diffusion.training_losses(
                    gen,
                    hq,
                    t,
                    model_kwargs=model_inputs,
            ...
            if len(self.extra_model_output_keys) > 0:
                assert len(self.extra_model_output_keys) == len(
                    diffusion_outputs["extra_outputs"]
                )
                out = {
                    k: v
                    for k, v in zip(
...
class GaussianDiffusionInferenceInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.noise_style = opt_get(
            opt, ["noise_type"], "random"
    ...
    def forward(self, state):
        if self.use_ema_model:
            ...
            gen = self.env["generators"][self.opt["generator"]]
        model_inputs = {
            k: state[v][: self.output_batch_size]
            for k, v in self.model_input_keys.items()
        }
        gen.eval()
        with torch.no_grad():
            if "low_res" in model_inputs.keys():
                output_shape = (
                    ...
                    3,
                    model_inputs["low_res"].shape[-2] * self.output_scale_factor,
                ...
                output_shape = (
                    ...
                    1,
                    closest_multiple(
                        model_inputs["spectrogram"].shape[-1]
                        * self.output_scale_factor,
                        self.multiple_requirement,
                    ),
                )
                ...
                output_shape = (
                    ...
                    1,
                    closest_multiple(
                        model_inputs["discrete_spectrogram"].shape[-1] * 1024,
                        self.multiple_requirement,
                    ),
                )
            ...
            if self.noise_style == "zero":
                ...
                if (
                    not hasattr(self, "fixed_noise")
                    or self.fixed_noise.shape != output_shape
                ):
                    self.fixed_noise = torch.randn(output_shape, device=dev)
                noise = self.fixed_noise
            gen = self.sampling_fn(
                gen,
                output_shape,
                noise=noise,
                model_kwargs=model_inputs,
