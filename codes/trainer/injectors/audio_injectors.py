    ...
    def forward(self, state):
        ...
        if torch.distributed.get_world_size() > 1:
            ...
            if self.rand_buffer_ptr >= self.rand_buffer_sz:
                self.rand_buffer = torch.randint(
                    self.min_crop_sz,
                    self.max_crop_sz,
                    (self.rand_buffer_sz,),
                    dtype=torch.long,
...
class AudioResampleInjector(Injector):
    ...
    def forward(self, state):
        inp = state[self.input]
        return {
            self.output: torchaudio.functional.resample(
                inp, self.input_sr, self.output_sr
            )
        }
...
class DiscreteTokenInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.dvae = load_model_from_config(
            cfg, dvae_name, device=f'cuda:{env["device"]}'
        ).eval()
...
class GptVoiceLatentInjector(Injector):
    ...
    def forward(self, state):
        with torch.no_grad():
            ...
            latents = self.gpt(
                mel_conds,
                state[self.text_input_key],
                state[self.text_lengths_key],
                codes,
                state[self.input_lengths_key],
                text_first=True,
                raw_mels=None,
    ...
    def forward(self, state):
        with torch.no_grad():
            ...
            if self.do_augmentations:
                original_audio = (
                    original_audio
                    + torch.rand_like(original_audio) * random.random() * 0.005
                )
                decoded_mel = (
                    decoded_mel + torch.rand_like(decoded_mel) * random.random() * 0.005
                )
                if random.random() < 0.5:
                    original_audio = torchaudio.functional.resample(
                        torchaudio.functional.resample(original_audio, 24000, 10000),
                        10000,
                        24000,
                    )
                if random.random() < 0.5:
                    decoded_mel = torchaudio.functional.resample(
                        torchaudio.functional.resample(decoded_mel, 24000, 10000),
                        10000,
                        24000,
                    )
                if random.random() < 0.5:
            ...
            labels = torch.rand(mel.shape[0], 1, 1, device=mel.device) > 0.5
...
class ConditioningLatentDistributionDivergenceInjector(Injector):
    def __init__(self, opt, env):
        ...
        if "gpt_config" in opt.keys():
            # The unified_voice model.
            cfg = opt_get(
                opt, ["gpt_config"], "../experiments/train_gpt_tts_unified.yml"
            )
            model_name = opt_get(opt, ["gpt_name"], "gpt")
            pretrained_path = opt["gpt_path"]
            self.latent_producer = load_model_from_config(
                cfg,
                model_name=model_name,
                also_load_savepoint=False,
                load_path=pretrained_path,
            ...
            self.latent_producer = DiffusionTtsFlat(
                model_channels=1024,
                num_layers=10,
                in_channels=100,
                out_channels=200,
...
class MusicCheaterArInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.cheater_ar.load_state_dict(
            torch.load(
