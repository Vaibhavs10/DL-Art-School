...
def test_mel_mask():
    ...
    a = load_audio(
        "D:\\data\\audio\\libritts\\test-clean\\61\\70970\\61_70970_000007_000001.wav",
        22050,
    ).unsqueeze(0)
...
class CombineMelInjector(Injector):
    ...
    def forward(self, state):
        ...
        return {
            self.output_audio_key: torch.stack(combined_audios, dim=0),
            self.output_text_key: torch.stack(combined_texts, dim=0),
        }


def test_mel_injector():
    inj = CombineMelInjector(
        {
            "audio_key": "a",
            "text_key": "t",
            "audio_lengths_key": "alk",
            "text_lengths_key": "tlk",
        },
        {},
    )
