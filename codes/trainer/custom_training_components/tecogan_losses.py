...
class RecurrentImageGeneratorSequenceInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.input_lq_index = (
            opt["input_lq_index"] if "input_lq_index" in opt.keys() else 0
        )
        self.recurrent_index = opt["recurrent_index"]
        self.output_hq_index = (
            opt["output_hq_index"] if "output_hq_index" in opt.keys() else 0
        )
        self.output_recurrent_index = (
            opt["output_recurrent_index"]
            if "output_recurrent_index" in opt.keys()
            else self.output_hq_index
        )
        self.scale = opt["scale"]
        self.resample = Resample2d()
        self.flow_key = (
            opt["flow_input_key"] if "flow_input_key" in opt.keys() else None
        )
        self.first_inputs = (
            opt["first_inputs"] if "first_inputs" in opt.keys() else opt["in"]
        )  # Use this to specify inputs that will be used in the first teco iteration, the rest will use 'in'.
        self.do_backwards = (
            opt["do_backwards"] if "do_backwards" in opt.keys() else True
        )
        self.hq_recurrent = (
            opt["hq_recurrent"] if "hq_recurrent" in opt.keys() else False
        ...
        self.hq_batched_output_key = (
            opt["hq_batched_key"] if "hq_batched_key" in opt.keys() else None
        )
    ...
    def forward(self, state):
        ...
        for i in range(f):
            if first_step:
                ...
                with torch.no_grad() and autocast(enabled=False):
                    ...
                    flow_input = torch.stack(
                        [flow_input, reduced_recurrent], dim=2
                    ).float()
            ...
            if self.env["step"] % 50 == 0:
                if (
                    input[self.input_lq_index].shape[1] == 3
                ):  # Only debug this if we're dealing with images.
                    self.produce_teco_visual_debugs(
                        input[self.input_lq_index],
                        input[self.hq_recurrent],
                        debug_index,
                    )
                    debug_index += 1
    ...
    def produce_teco_visual_debugs(self, gen_input, gen_recurrent, it):
        ...
        torchvision.utils.save_image(
            gen_recurrent.float(), osp.join(base_path, "%s_recurrent.png" % (it,))
        )
...
def create_teco_discriminator_sextuplet(
    input_list, lr_imgs, scale, index, flow_gen, resampler, margin
):
    ...
    with autocast(enabled=False):
        ...
        flow_triplet = [
            resampler(triplet[:, 0], first_flow),
            triplet[:, 1],
...
def create_all_discriminator_sextuplets(
    input_list, lr_imgs, scale, total, flow_gen, resampler, margin
):
    with autocast(enabled=False):
        ...
        flux_doubles_forward = [
            torch.stack([input_list[:, i], input_list[:, i + 1]], dim=2)
...
class TecoGanLoss(ConfigurableLoss):
    ...
    def compute_loss(self, real_sext, fake_sext):
        ...
        if self.opt["gan_type"] in ["gan", "pixgan"]:
            ...
            l_step = self.criterion(
                d_real - torch.mean(d_fake), not self.for_generator
    ...
    def produce_teco_visual_debugs(self, sext, lbl, it):
        ...
        for i in range(6):
            torchvision.utils.save_image(
                sext[:, i * 3 : (i + 1) * 3, :, :].float(),
                osp.join(base_path, "%s_%s.png" % (it, lbls[i])),
            )
