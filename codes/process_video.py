...
class FfmpegBackedVideoDataset(data.Dataset):
    ...
    def __init__(self, opt, working_dir):
        ...
        self.vertical_splits = (
            self.opt["vertical_splits"] if "vertical_splits" in opt.keys() else 1
        )
    ...
    def __getitem__(self, index):
        ...
        working_file_name = osp.join(
            self.working_dir,
            "working_%d.png" % (actual_index % self.max_working_files,),
        )
        ...
        return {
            ...
            "lq_center": torch.tensor(
                [img_LQ.shape[1] // 2, img_LQ.shape[2] // 2], dtype=torch.long
            ),
        }
...
if __name__ == "__main__":
    ...
    vid_output = (
        opt["mini_vid_output_folder"]
        if "mini_vid_output_folder" in opt.keys()
        else dataset_dir
    )
    ...
    for data in tq:
        ...
        if recurrent_mode and first_frame:
            ...
            if "recurrent_hr_generator" in opt.keys():
                ...
                model.env["generators"]["generator"] = model.env["generators"][
                    opt["recurrent_hr_generator"]
                ]
        ...
        for i in range(visuals.shape[0]):
            ...
            if frame_counter % frames_per_vid == 0:
                ...
                # Perform stitching.
                num_splits = (
                    opt["dataset"]["vertical_splits"]
                    if "vertical_splits" in opt["dataset"].keys()
                    else 1
                )
                if num_splits > 1:
                    procs = []
                    ...
                    for i in range(int(frames_per_vid / num_splits)):
                        to_join = [
                            osp.join(dataset_dir, "%08d.png" % (j,))
                            for j in range(i * num_splits, i * num_splits + num_splits)
                        ]
                        merge_images(
                            to_join, osp.join(src_imgs_path, "%08d.png" % (i,))
                        )
                else:
                ...
                cmd = [
                    ...
                    "-y",
                    "-framerate",
                    str(opt["dataset"]["frame_rate"]),
                    "-f",
                    "image2",
                    "-i",
                    osp.join(src_imgs_path, "%08d.png"),
                    "-c:v",
                    "libx265",
                    "-crf",
                    str(minivid_crf),
                    ...
                    "-pix_fmt",
                    "yuv444p",
                    osp.join(vid_output, "mini_%06d.mkv" % (vid_counter,)),
                ]
                print(ffmpeg_proc)
