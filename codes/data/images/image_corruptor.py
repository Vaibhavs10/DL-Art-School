...
class ImageCorruptor:
    def __init__(self, opt):
        ...
        self.reset_random()
        self.blur_scale = (
            opt["corruption_blur_scale"] if "corruption_blur_scale" in opt.keys() else 1
        )
        self.fixed_corruptions = (
            opt["fixed_corruptions"] if "fixed_corruptions" in opt.keys() else []
        )
        self.num_corrupts = (
            opt["num_corrupts_per_image"]
            if "num_corrupts_per_image" in opt.keys()
            else 0
        )
        ...
        if self.num_corrupts == 0:
            ...
            self.random_corruptions = (
                opt["random_corruptions"] if "random_corruptions" in opt.keys() else []
            )
        ...
        if "color_quantization" in aug:
            ...
            k = cv2.warpAffine(
                ...
                cv2.getRotationMatrix2D(
                    (intensity / 2 - 0.5, intensity / 2 - 0.5), angle, 1.0
                ),
                (intensity, intensity),
            )
            k = k * (1.0 / np.sum(k))
            ...
            if (
                "noise" not in applied_augmentations
                and "noise-5" not in applied_augmentations
            ):
                if aug == "jpeg":
                    lo = 10
                    range = 20
