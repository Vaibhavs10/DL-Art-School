...
class SingleImageDataset(BaseUnsupervisedImageDataset):
    ...
    def get_paths(self):
        for i in range(len(self)):
            ...
            chunk_ind = (
                ...
                and self.starting_indices[chunk_ind] == i
                else chunk_ind - 1
            )
    ...
    def __getitem__(self, item):
        ...
        hq, hq_ref, hq_center, hq_mask, path = self.chunks[chunk_ind][
            item - self.starting_indices[chunk_ind]
        ]
        ...
        hq_ref = torch.from_numpy(
            np.ascontiguousarray(np.transpose(hrs[0], (2, 0, 1)))
        ).float()
        hq_mask = torch.from_numpy(np.ascontiguousarray(hms[0])).unsqueeze(dim=0)
        hq_ref = torch.cat([hq_ref, hq_mask], dim=0)
        lq = torch.from_numpy(
            np.ascontiguousarray(np.transpose(ls[0], (2, 0, 1)))
        ).float()
        lq_ref = torch.from_numpy(
            np.ascontiguousarray(np.transpose(lrs[0], (2, 0, 1)))
        ).float()
        ...
        return {
            ...
            "lq_center": torch.tensor(lcs[0], dtype=torch.long),
            "gt_center": torch.tensor(hcs[0], dtype=torch.long),
            "LQ_path": path,
            "GT_path": path,
        }
