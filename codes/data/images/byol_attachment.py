...
class ByolDatasetWrapper(Dataset):
    ...
    def __getitem__(self, item):
        ...
        item.update(
            {
                "aug1": self.aug(item[self.key1]).squeeze(dim=0),
                "aug2": self.aug(item[self.key2]).squeeze(dim=0),
            }
        )
        return item
    ...
    for i in tqdm(range(0, len(ds))):
        ...
        for k, v in o.items():
            ...
            if k in ["hq"]:
                ...
                masked = v * (o["labels_mask"] * 0.5 + 0.5)
                ...
                if len(o["labels"].unique()) > 1:
                    ...
                    moremask = v * ((1 * (o["labels"] == randlbl)) * 0.5 + 0.5)
                    torchvision.utils.save_image(
                        moremask.unsqueeze(0),
                        "debug/%i_%s_%s.png" % (i, k, o["label_strings"][randlbl]),
                    )
...
class RandomSharedRegionCrop(nn.Module):
    ...
    def forward(self, i1, i2):
        ...
        # Step 4
        m = self.multiple
        jl, jt = random.randint(-self.jitter_range, self.jitter_range), random.randint(
            -self.jitter_range, self.jitter_range
        )
        jt = (
            jt if base_t != 0 else abs(jt)
        )  # If the top of a patch is zero, a negative jitter will cause it to go negative.
        jt = (
            jt if (base_t + base_h) * m != i1.shape[1] else 0
        ...
        p1 = i1[
            :,
            base_t * m + jt : (base_t + base_h) * m + jt,
        ...
        recompute_package = torch.tensor(
            [
                d,
                base_h,
                base_w,
        ...
        # Step 7
        mask1 = torch.full((1, base_h * m, base_w * m), fill_value=0.5)
        mask1[
            :,
            i1_shared_t * m : (i1_shared_t + ix_h) * m,
            i1_shared_l * m : (i1_shared_l + ix_w) * m,
        ] = 1
        masked1 = pad_to(p1 * mask1, d * m)
        mask2 = torch.full((1, im2_h * m, im2_w * m), fill_value=0.5)
        mask2[
            :,
            i2_shared_t * m : (i2_shared_t + ix_h) * m,
            i2_shared_l * m : (i2_shared_l + ix_w) * m,
        ] = 1
        masked2 = pad_to(p2 * mask2, d * m)
        mask = torch.full((1, d * m, d * m), fill_value=0.33)
        mask[
            :, base_t * m : (base_t + base_w) * m, base_l * m : (base_l + base_h) * m
        ] += 0.33
        mask[
            :, im2_t * m : (im2_t + im2_w) * m, im2_l * m : (im2_l + im2_h) * m
        ] += 0.33
        masked_dbg = i1 * mask

        # Step 8 - Rebuild shared regions for testing purposes.
        p1_shuf, p2_shuf = PixelUnshuffle(self.multiple)(
            p1_resized.unsqueeze(0)
        ...
        item.update(
            {
                "aug1": a1,
                "aug2": a2,
                "similar_region_dimensions": sr_dim,
                "masked1": m1,
                "masked2": m2,
                "aug_shared_view": db,
                "i1_shared": i1s,
...
def test_structured_crop_dataset_wrapper():
    ...
    for i in tqdm(range(0, len(ds))):
        ...
        rc1, rc2 = reconstructed_shared_regions(
            pixun(o["aug1"].unsqueeze(0)),
            pixun(o["aug2"].unsqueeze(0)),
            rcpkg.unsqueeze(0),
        )
