...
class VsNetImageLabeler:
    ...
    def get_labels_as_tensor(self, hq, img_key, resize_factor):
        ...
        for patch_lbl in lbl_list:
            t, l, h, w = (
                patch_lbl["patch_top"] // resize_factor,
                patch_lbl["patch_left"] // resize_factor,
                patch_lbl["patch_height"] // resize_factor,
                patch_lbl["patch_width"] // resize_factor,
            )
    ...
    def add_label(self, binding, img_name, top, left, dim):
        lbl = {
            "path": img_name,
            "label": self.categories[binding]["label"],
            "patch_top": top,
            "patch_left": left,
            "patch_height": dim,
            "patch_width": dim,
...
class CompactJsonLabeler:
    ...
    def get_labels_as_tensor(self, hq, img_key, resize_factor):
        ...
        for patch_lbl in lbl_list:
            t, l, h, w = (
                patch_lbl["top"] // resize_factor,
                patch_lbl["left"] // resize_factor,
                self.config["dim"] // resize_factor,
                self.config["dim"] // resize_factor,
            )
            val = patch_lbl["labelValue"]
            labels[:, t : t + h, l : l + w] = val
