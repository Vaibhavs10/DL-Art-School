...
class MultiScaleDataset(data.Dataset):
    ...
    def recursively_extract_patches(self, input_img, result_list, depth):
        ...
        result_list.extend(
            [
                cv2.resize(
                    p, (self.tile_size, self.tile_size), interpolation=cv2.INTER_AREA
                )
                for p in patches
            ]
        )
    ...
    def __getitem__(self, index):
        ...
        if img_full3.shape[0] == 0 or img_full3.shape[1] == 0:
            print(
                ...
                % (full_path, str(loaded_img.shape)),
                str(img_full1.shape),
                str(img_full2.shape),
                str(img_full3.shape),
            )
        ...
        img_full = cv2.resize(
            img_full3,
            (self.hq_size_cap, self.hq_size_cap),
        ...
        if patches_hq[0].shape[2] == 3:
            ...
            patches_hq_corrupted = [
                cv2.cvtColor(p, cv2.COLOR_BGR2RGB) for p in patches_hq_corrupted
            ]
        patches_hq = [
            torch.from_numpy(np.ascontiguousarray(np.transpose(p, (2, 0, 1)))).float()
            for p in patches_hq
        ]
        ...
        patches_lq = [
            torch.nn.functional.interpolate(
                p.unsqueeze(0), scale_factor=1 / self.scale, mode="area"
            ).squeeze()
            for p in patches_hq_corrupted
        ]
        ...
        for k, v in o.items():
            ...
            while node is not None:
                torchvision.utils.save_image(
                    v[node.index].unsqueeze(0), "debug/%i_%s_%i.png" % (i, k, depth)
                )
                depth += 1
                node = node.parent
