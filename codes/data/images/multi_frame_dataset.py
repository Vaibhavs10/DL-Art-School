...
class MultiFrameDataset(BaseUnsupervisedImageDataset):
    ...
    def get_sequential_image_paths_from(self, chunk_index, chunk_offset):
        ...
        for i in range(self.num_frames):
            ...
            if (
                idx < 0
                or idx >= len(self.chunks)
                or chunk_offset < 0
                or chunk_offset >= len(self.chunks[idx])
            ):
                print(
                    "Chunk reference indexing failed for %s." % (im_name,),
                    search_idx,
                    i,
                    chunk_offset,
                    self.num_frames,
    ...
    def __getitem__(self, item):
        ...
        chunk_ind = (
            chunk_ind
            if chunk_ind < len(self.starting_indices)
            and self.starting_indices[chunk_ind] == item
            else chunk_ind - 1
        )
        ...
        hq = torch.from_numpy(
            np.ascontiguousarray(np.transpose(np.stack(hs), (0, 3, 1, 2)))
        ).float()
        hq_ref = torch.from_numpy(
            np.ascontiguousarray(np.transpose(np.stack(hrs), (0, 3, 1, 2)))
        ).float()
        hq_mask = torch.from_numpy(np.ascontiguousarray(np.stack(hms))).unsqueeze(dim=1)
        hq_ref = torch.cat([hq_ref, hq_mask], dim=1)
        lq = torch.from_numpy(
            np.ascontiguousarray(np.transpose(np.stack(ls), (0, 3, 1, 2)))
        ).float()
        lq_ref = torch.from_numpy(
            np.ascontiguousarray(np.transpose(np.stack(lrs), (0, 3, 1, 2)))
        ).float()
        ...
        return {
            ...
            "lq": lq,
            "hq": hq,
            "gt_fullsize_ref": hq_ref,
            "lq_fullsize_ref": lq_ref,
            "lq_center": torch.tensor(lcs, dtype=torch.long),
            "gt_center": torch.tensor(hcs, dtype=torch.long),
        }
