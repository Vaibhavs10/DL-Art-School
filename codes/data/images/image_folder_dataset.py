...
class ImageFolderDataset:
    def __init__(self, opt):
        ...
        if "center_crop_hq_sz" in opt.keys():
            self.center_crop = functools.partial(
                ndarray_center_crop, opt["center_crop_hq_sz"]
            )
        self.target_hq_size = (
            opt["target_size"] if "target_size" in opt.keys() else None
        )
        self.multiple = opt["force_multiple"] if "force_multiple" in opt.keys() else 1
        self.scale = opt["scale"]
        self.paths = opt["paths"]
        self.corrupt_before_downsize = (
            opt["corrupt_before_downsize"]
            if "corrupt_before_downsize" in opt.keys()
            else False
        )
        self.fetch_alt_image = opt[
            "fetch_alt_image"
        ]  # If specified, this dataset will attempt to find a second image
        # from the same video source. Search for 'fetch_alt_image' for more info.
        self.fetch_alt_tiled_image = opt[
            "fetch_alt_tiled_image"
        ]  # If specified, this dataset will attempt to find anoter tile from the same source image
        #  Search for 'fetch_alt_tiled_image' for more info.
        assert not (
            self.fetch_alt_image and self.fetch_alt_tiled_image
        ...
        if "labeler" in opt.keys():
            ...
            assert (
                len(self.paths) == 1
            )  # Only a single base-path is supported for labeled images.
    ...
    def resize_hq(self, imgs_hq):
        ...
        if self.target_hq_size is not None and self.target_hq_size != h:
            ...
            for hq in imgs_hq:
                ...
                hqs_adjusted.append(
                    cv2.resize(hq, target_size, interpolation=cv2.INTER_AREA)
                )
        ...
        if self.corrupt_before_downsize:
            ...
            if "corrupt_before_downsize_factor" in self.opt.keys():
                ...
                hs = [
                    cv2.resize(
                        h_,
                        (h // special_factor, w // special_factor),
    ...
    def __getitem__(self, item):
        ...
        if not self.disable_flip and random.random() < 0.5:
            hq = hq[:, ::-1, :]
        ...
        hq = torch.from_numpy(
            np.ascontiguousarray(np.transpose(hs[0], (2, 0, 1)))
        ).float()

        out_dict = {
            "hq": hq,
            "LQ_path": self.image_paths[item],
            "HQ_path": self.image_paths[item],
            "has_alt": False,
        }
        ...
        if self.fetch_alt_image:
            ...
            while ".jpg.jpg" in imname_parts:
                imname_parts = imname_parts.replace(
                    ".jpg.jpg", ".jpg"
                )  # Hack workaround to my own bug.
            ...
            if len(imname_parts) >= 2 and len(imname_parts[-2]) > 8:
                try:
                    ...
                    next_img = self.image_paths[item].replace(
                        str(imnumber), str(imnumber + 1)
                    )
                    alt_hq = util.read_img(None, next_img, rgb=True)
                    alt_hs = self.resize_hq([alt_hq])
                    alt_hq = torch.from_numpy(
                        np.ascontiguousarray(np.transpose(alt_hs[0], (2, 0, 1)))
                    ).float()
        ...
        if self.fetch_alt_tiled_image:
            ...
            try:
                if len(other_images) <= 3:
                    alt_hq = (
                        hq  # This is a fallback in case an alt image can't be found.
                    )
                else:
                    random.shuffle(other_images)
                    for oi in other_images:
                        if (
                            oi == os.path.basename(self.image_paths[item])
                            or "ref." in oi
                            or "centers.pt" in oi
                        ):
                            continue
                        alt_hq = util.read_img(
                            None, os.path.join(sel_path, oi), rgb=True
                        )
                        alt_hs = self.resize_hq([alt_hq])
                        alt_hq = torch.from_numpy(
                            np.ascontiguousarray(np.transpose(alt_hs[0], (2, 0, 1)))
        ...
        if not self.skip_lq:
            ...
            ls = lqs[0]
            out_dict["lq"] = torch.from_numpy(
                np.ascontiguousarray(np.transpose(ls, (2, 0, 1)))
            ).float()
            out_dict["corruption_entropy"] = torch.tensor(ent)
            if len(lqs) > 1:
                alt_lq = lqs[1]
                out_dict["alt_lq"] = torch.from_numpy(
                    np.ascontiguousarray(np.transpose(alt_lq, (2, 0, 1)))
                ).float()
        ...
        if self.labeler:
            ...
            lbls, lbl_masks, lblstrings = self.labeler.get_labels_as_tensor(
                hq, base_file, dim // hq.shape[1]
            )
