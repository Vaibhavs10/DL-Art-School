...
class BaseUnsupervisedImageDataset(data.Dataset):
    ...
    def resize_hq(self, imgs_hq, refs_hq, masks_hq, centers_hq):
        ...
        if self.target_hq_size is not None and self.target_hq_size != h:
            ...
            for hq, hq_ref, hq_mask, hq_center in zip(
                imgs_hq, refs_hq, masks_hq, centers_hq
            ):
                ...
                hq_refs_adjusted.append(
                    cv2.resize(hq_ref, target_size, interpolation=cv2.INTER_AREA)
                )
                hq_masks_adjusted.append(
                    cv2.resize(hq_mask, target_size, interpolation=cv2.INTER_AREA)
                )
            ...
            hq_masks_adjusted = [
                m.squeeze(-1) for m in hq_masks_adjusted
