...
def load_similar_clips(path, sample_length, sample_rate, n=3, fallback_to_self=True):
    ...
    if os.path.exists(sim_path):
        ...
        if fname in similarities.keys():
            candidates = [
                os.path.join(os.path.dirname(path), s) for s in similarities[fname]
            ]
        else:
            print(
...
class UnsupervisedAudioDataset(torch.utils.data.Dataset):
    ...
    def get_related_audio_for_index(self, index):
        if self.extra_samples <= 0:
            return None, 0
        audiopath = self.audiopaths[index]
        return load_similar_clips(
            audiopath, self.extra_sample_len, self.sampling_rate, n=self.extra_samples
        )
...
if __name__ == "__main__":
    params = {
        ...
        "path": [
            "Y:\\separated\\yt-music-0",
            "Y:\\separated\\yt-music-1",
            "Y:\\separated\\bt-music-1",
            "Y:\\separated\\bt-music-2",
            "Y:\\separated\\bt-music-3",
            "Y:\\separated\\bt-music-4",
    ...
    for b in tqdm(dl):
        for b_ in range(b["clip"].shape[0]):
            ...
            torchaudio.save(
                f"{i}_alt_clip_{b_}.wav", b["alt_clips"][b_], ds.sampling_rate
            )
            i += 1
        if i > 200:
            break
