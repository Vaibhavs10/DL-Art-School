...
from models.audio.tts.tacotron2 import (
...
def load_tsv_aligned_codes_type(filename, type):
    with open(filename, encoding="utf-8") as f:
        ...
        for line in f:
            ...
            filepaths_and_text.append(
                [
                    os.path.join(base, f"{components[1]}"),
                    components[0],
                    convert_string_list_to_tensor(components[2]),
                ]
                + [type]
            )
    return filepaths_and_text
...
def load_mozilla_cv(filename, type):
    with open(filename, encoding="utf-8") as f:
        ...
        base = os.path.dirname(filename)
        filepaths_and_text = [
            [os.path.join(base, f"clips/{component[1]}"), component[2], type]
    ...
    def __init__(self, hparams):
        ...
        for p, fm, type in zip(self.path, fetcher_mode, self.types):
            if fm == "lj" or fm == "libritts":
                ...
                fetcher_fn = (
                    load_tsv_aligned_codes_type
                    if self.load_aligned_codes
                    else load_tsv_type
                )
            elif fm == "mozilla_cv":
                assert (
                    not self.load_conditioning
        ...
        if self.max_wav_len is not None:
            self.max_aligned_codes = (
                self.max_wav_len // self.aligned_codes_to_audio_ratio
            )
        ...
        self.skipped_items = (
            0  # records how many items are skipped when accessing an index.
        )
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if text is None or len(text.strip()) == 0:
                raise ValueError
            if wav is None or wav.shape[-1] < (0.6 * self.sample_rate):
            ...
            cond, cond_is_self = (
                load_similar_clips(
                    ...
                    self.sample_rate,
                    n=self.conditioning_candidates,
                )
...
class PairedVoiceDebugger:
    ...
    def update(self, batch):
        ...
        if "conditioning" in batch.keys():
            self.self_conditioning_items += (
                batch["conditioning_contains_self"].sum().item()
            )

    def get_debugging_map(self):
        return {
            "total_samples_loaded": self.total_items,
            "percent_skipped_samples": (self.loaded_items - self.total_items)
            / self.loaded_items,
            "percent_conditioning_is_self": self.self_conditioning_items
            / self.loaded_items,
        }
