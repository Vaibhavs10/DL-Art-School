...
def load_tsv(filename):
    with open(filename, encoding="utf-8") as f:
        ...
        filepaths_and_text = [
            [os.path.join(base, f"{component[1]}"), component[0]]
...
def load_mozilla_cv(filename):
    with open(filename, encoding="utf-8") as f:
        ...
        base = os.path.dirname(filename)
        filepaths_and_text = [
            [os.path.join(base, f"clips/{component[1]}"), component[2]]
            for component in components
        ]
    return filepaths_and_text
...
class TextWavLoader(torch.utils.data.Dataset):
    ...
    def load_conditioning_candidates(self, path):
        ...
        for k in range(self.conditioning_candidates):
            ...
            if gap < 0:
                ...
                rel_clip = rel_clip[
                    :, rand_start : rand_start + self.conditioning_length
                ]
    ...
    def __getitem__(self, index):
        try:
            ...
            cond = (
                self.load_conditioning_candidates(self.audiopaths_and_text[index][0])
