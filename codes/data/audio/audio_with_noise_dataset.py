...
class AudioWithNoiseDataset(Dataset):
    def __init__(self, opt):
        ...
        self.openair_paths = find_files_of_type(
            "img", opt["openair_path"], qualifier=is_audio_file
        )[0]
        self.min_volume = opt_get(opt, ["min_noise_volume"], 0.2)
        self.max_volume = opt_get(opt, ["max_noise_volume"], 0.5)
    ...
    def load_openair_kernels(self):
        if self.use_gpu_for_reverb_compute and self.openair_kernels is None:
            ...
            for oa in self.openair_paths:
                self.openair_kernels.append(
                    load_rir(
                        oa,
                        self.underlying_dataset.sampling_rate,
                        self.underlying_dataset.sampling_rate * 2,
                    ).cuda()
                )
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            clipvol = random.random() * (0.8 - 0.5) + 0.5
            clip = clip * clipvol
            ...
            if label > 0 and label < 4:  # 0 is basically "leave it alone"
                aug_needed = True
                augvol = (
                    random.random() * (self.max_volume - self.min_volume)
                    + self.min_volume
                )
                if label == 1:
                ...
                if self.use_gpu_for_reverb_compute:
                    clip = clip.cuda()
                clip = (
                    torch.nn.functional.conv1d(clip.unsqueeze(0), rir.unsqueeze(0))
                    .squeeze(0)
                    .cpu()
                )
            elif label == 5:
...
if __name__ == "__main__":
    params = {
        ...
        "extra_samples": 0,
        "env_noise_paths": [
            "E:\\audio\\UrbanSound\\filtered",
            "E:\\audio\\UrbanSound\\MSSND",
        ],
        "env_noise_cache": "E:\\audio\\UrbanSound\\cache.pth",
        "music_paths": [
            "E:\\audio\\music\\FMA\\fma_large",
            "E:\\audio\\music\\maestro\\maestro-v3.0.0",
        ],
