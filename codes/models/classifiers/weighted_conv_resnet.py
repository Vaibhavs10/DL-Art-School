...
def conv1x1(
    in_planes: int, out_planes: int, stride: int = 1, breadth: int = 8
) -> ScaledWeightConv:
    """1x1 convolution"""
    return ScaledWeightConv(
        in_planes, out_planes, kernel_size=1, stride=stride, bias=False, breadth=breadth
    )
...
class MaskedSequential(nn.Module):
    ...
    def forward(self, x):
        ...
        for m in self:
            if (
                isinstance(m, ScaledWeightConv)
                or isinstance(m, BasicBlock)
                or isinstance(m, Bottleneck)
            ):
                x = m(x, mask)
            else:
                x = m(x)
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = MaskedSequential(
                conv1x1(
                    self.inplanes, planes * block.expansion, stride, breadth=breadth
                ),
