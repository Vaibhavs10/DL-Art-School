    ...
    def forward(self, text, speech_tokens, return_loss=False):
        ...
        if self.training:
            if self.clip_mels:
                ...
                voice_mask = torch.ones_like(
                    speech_tokens.float()
                ).bool()  # Disable voice masking in this case.
            else:
                voice_mask = (
        ...
        if self.distributed_collect:
            ...
            ] = text_latents  # For gradient propagation.
            text_latents = torch.cat(collective, dim=0)
            collective = [
                torch.zeros_like(speech_latents)
                for _ in range(torch.distributed.get_world_size())
            ]
            collective[
                torch.distributed.get_rank()
            ] = speech_latents  # For gradient propagation.
...
if __name__ == "__main__":
    clip = VoiceCLIP(
        text_mask_percentage=0.2, voice_mask_percentage=0.2, use_xformers=True
    )
    clip(
