...
class FeedForward(nn.Module):
    def __init__(
        ...
        project_in = (
            nn.Sequential(mbnb.nn.Linear(dim, inner_dim), activation)
            if not glu
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_out = (
            nn.Sequential(mbnb.nn.Linear(v_dim, out_dim * 2), nn.GLU())
            if on_attn
            else mbnb.nn.Linear(v_dim, out_dim)
        )
...
class TransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.project_emb = (
            mbnb.nn.Linear(emb_dim, dim) if emb_dim != dim else nn.Identity()
        )
        self.attn_layers = attn_layers
        self.norm = nn.LayerNorm(dim)
...
class ContinuousTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.project_in = (
            mbnb.nn.Linear(dim_in, dim) if exists(dim_in) else nn.Identity()
        )
        ...
        self.project_out = (
            mbnb.nn.Linear(dim, dim_out) if exists(dim_out) else nn.Identity()
        )
