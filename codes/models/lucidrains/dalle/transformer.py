...
from models.lucidrains.dalle.attention import (
    Attention,
    SparseAttention,
...
class Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        for ind, sparse_attn, attn_type in zip(
            range(depth), sparse_layer, attn_type_layer
        ):
            if attn_type == "full":
            ...
            if shift_tokens:
                attn, ff = map(
                    lambda t: PreShiftToken(
                        t, image_size=image_fmap_size, seq_len=seq_len
                    ),
                    (attn, ff),
                )
        ...
        if rotary_emb:
            ...
            img_seq_len = (
                (image_fmap_size**2)
                if image_fmap_size is not None
                else oned_fmap_size
            )
            text_len = seq_len - img_seq_len + 1
