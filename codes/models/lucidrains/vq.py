    ...
    def forward(self, x, used_codes=[]):
        ...
        for uc in used_codes:
            mask = torch.arange(0, self.codebook_size, device=x.device).unsqueeze(
                0
            ).repeat(x.shape[0], 1) == uc.unsqueeze(1)
            dist[mask] = -torch.inf
...
class CosineSimCodebook(nn.Module):
    ...
    def init_embed_(self, data):
        ...
        embed, cluster_size = kmeans(
            data, self.codebook_size, self.kmeans_iters, use_cosine_sim=True
        )
        self.embed.data.copy_(embed)
        self.cluster_size.data.copy_(cluster_size)
        self.initted.data.copy_(torch.Tensor([True]))
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        commitment=1.0,  # deprecate in next version, turn off by default
