...
class Wav2VecWrapper(nn.Module):
    ...
    def __init__(
        ...
        ramp_dropout_min=0.1,
        ramp_dropout_max=0.5,
        layer_drop_pct=0.1,
    ):
        super().__init__()
        ...
        if freeze_transformer:
            ...
            for p in list(self.w2v.wav2vec2.encoder.parameters()) + list(
                self.w2v.wav2vec2.feature_projection.parameters()
            ):
                p.requires_grad = False
                p.DO_NOT_TRAIN = True
    ...
    def forward(
        self, audio, unaligned_tokens, wav_lengths, text_lengths, fea_extractor=None
    ):
    ...
    def get_debug_values(self, step, net_name):
        ...
        if self.output_wer and step % 100 == 0:
            ...
            for last_labels, last_pred in zip(self.last_labels, self.last_pred):
                ...
                pred_strings.extend(
                    [
                        only_letters(sequence_to_text(self.decode_ctc(pred)))
    ...
    def update_for_step(self, step, *args):
        if self.ramp_dropout_mode and step % 10 == 0:
            ...
            new_dropout_rate = self.ramp_dropout_min + dropout_gap * min(
                step / self.ramp_dropout_end, 1
            )
...
if __name__ == "__main__":
    ...
    w2v = Wav2VecWrapper(
        basis_model="facebook/wav2vec2-large-960h",
        freeze_transformer=True,
        remove_feature_extractor=True,
        ramp_dropout_mode=True,
    )
    w2v.update_for_step(8000)
    fea = fe(torch.randn(2, 1, 50000), torch.tensor([20000, 30000]))
    loss = w2v(
        torch.randn(2, 1, 50000),
        torch.randint(0, 40, (2, 70)),
        torch.tensor([20000, 30000]),
        torch.tensor([35, 50]),
        fea,
    )
