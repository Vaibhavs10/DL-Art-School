...
class UnifiedVoice(nn.Module):
    ...
    def text_forward(self, speech_conditioning_input, text_inputs, text_lengths):
        ...
        text_emb = (
            self.text_embedding(text_inputs)
            + self.text_pos_embedding(text_inputs)
    ...
    def speech_forward(
        self, speech_conditioning_input, mel_codes, wav_lengths, raw_mels=None
    ):
        """
    ...
    def convert_attentions_to_aligned_codes(self, text, attentions, codes, num_conds):
        ...
        for t, att_tok in enumerate(attentions):
            ...
            for lyr in att_tok:
                token_to_text_attentions = lyr[
                    :, :, -1, text_padding : (text_padding + num_text)
                ].sum(dim=1)
                combined_attention_weights = (
                    combined_attention_weights + token_to_text_attentions
                )
                break
