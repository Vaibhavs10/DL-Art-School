...
class UnifiedVoice(nn.Module):
    def __init__(
        ...
        # nn.Embedding
        self.text_embedding = mbnb.nn.Embedding(
    ...
    def get_logits(
        ...
        if mel_inputs is not None:
            emb = torch.cat(
                [speech_conditioning_inputs, text_inputs, mel_inputs], dim=1
            )
        else:
    ...
    def forward(
        ...
        text_logits, mel_logits, aligned_logits = self.get_logits(
            conds,
            text_emb,
            self.text_head,
            mel_emb,
    ...
    def inference_speech(
        self, speech_conditioning_input, text_inputs, **hf_generate_kwargs
    ):
        if (
            self.max_mel_tokens == -1
        ):  # Assume if this is the case, max_mel_tokens=-1 also
