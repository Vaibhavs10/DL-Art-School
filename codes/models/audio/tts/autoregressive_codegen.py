...
class ConditioningEncoder(nn.Module):
    def __init__(
        ...
        self.init = nn.Sequential(
            nn.Conv1d(spec_dim, embedding_dim // 4, kernel_size=5, padding=2),
            nn.Conv1d(
                embedding_dim // 4,
                embedding_dim // 2,
                kernel_size=3,
            ...
            nn.Conv1d(
                embedding_dim // 2, embedding_dim, kernel_size=3, padding=1, stride=2
            ),
        )
...
class AutoregressiveCodegen(nn.Module):
    def __init__(
        self, model_dim, depth, num_text_tokens=256, num_mel_tokens=8194, dropout=0.1
    ):
        super().__init__()
        assert (
            depth >= 8
    ...
    def forward(
        self, text_codes, conditioning_signal, mel_codes, wav_lengths, return_loss=True
    ):
        assert (
            text_codes.max() < self.max_text_token_id and text_codes.min() >= 0
        ), f"Invalid text code encountered: {text_codes.max()}, {text_codes.min()}"
        assert (
            mel_codes.max() < self.max_mel_token_id and mel_codes.min() >= 0
        ), f"Invalid mel code encountered: {mel_codes.max()}, {mel_codes.min()}"
    ...
    def generate(
        self, conditioning_signal, text_codes, max_tokens=256, **hf_generate_kwargs
    ):
