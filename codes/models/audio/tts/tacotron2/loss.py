...
class Tacotron2Loss(ConfigurableLoss):
    ...
    def forward(self, _, state):
        mel_target, gate_target = (
            state[self.mel_target_key],
            state[self.gate_target_key],
        )
        ...
        mel_out, mel_out_postnet, gate_out = (
            state[self.mel_output_key],
            state[self.mel_output_postnet_key],
            state[self.gate_output_key],
        )
    ...
    def extra_metrics(self):
        return {"mel_loss": self.last_mel_loss, "gate_loss": self.last_gate_loss}
