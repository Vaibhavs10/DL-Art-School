...
class WavDecoder(nn.Module):
    def __init__(
        self, dec_channels, K_ms=40, sample_rate=8000, dropout_probability=0.1
    ):
        super().__init__()
        self.dec_channels = dec_channels
        self.K = int(sample_rate * (K_ms / 1000))
        self.clarifier = UNetModel(
            image_size=self.K,
            in_channels=1,
            model_channels=dec_channels
            // 4,  # This is a requirement to enable to load the embedding produced by the decoder into the unet model.
        ...
        self.pre_rnn = nn.Sequential(
            ConvGnSilu(1, 32, kernel_size=5, convnd=nn.Conv1d),
    ...
    def produce_context(self, decoder_input):
        ...
        self.attention_context, self.attention_weights = checkpoint(
    ...
    def recombine(self, diffusion_eps, gate_outputs, alignments, padding_added):
        ...
        return (
            diffusion_eps[:, :, :-padding_added],
            gate_outputs[:, :-padding_added],
            alignments[:, :-padding_added],
        )
    ...
    def forward(self, wav_noised, wav_real, timesteps, text_enc, memory_lengths):
        ...
        wav_real = checkpoint(self.pre_rnn, wav_real.reshape(b * s, 1, K)).reshape(
            b, s, self.dec_channels
        )
        ...
        diffusion_eps = self.clarifier(
            wav_noised, timesteps.repeat(s), diffusion_emb
        ).reshape(b, s, 2, self.K)
...
if __name__ == "__main__":
    ...
    out = tron(
        wavs_diffused=torch.randn(2, 1, 22000),
