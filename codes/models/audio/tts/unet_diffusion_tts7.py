...
def clustered_mask(
    probability, shape, dev, lateral_expansion_radius_max=3, inverted=False
):
    """
    ...
    kernel = torch.tensor(
        [0.5 for _ in range(lateral_expansion_radius_max)]
        + [1]
        + [0.5 for _ in range(lateral_expansion_radius_max)],
        device=dev,
    )
    mask = F.conv1d(
        mask.unsqueeze(1),
        kernel.view(1, 1, 2 * lateral_expansion_radius_max + 1),
        padding=lateral_expansion_radius_max,
    ).squeeze(1)
    if inverted:
...
class DiffusionTts(nn.Module):
    ...
    def __init__(
        ...
        token_conditioning_resolutions=(
            1,
            16,
        ),
        ...
        nil_guidance_fwd_proportion=0.3,
        ...
        super_sampling_max_noising_factor=0.1,
        ...
        self.contextual_embedder = AudioMiniEncoder(
            1,
            embedding_dim,
            base_channels=32,
            depth=6,
        ...
        if enabled_unaligned_inputs:
            # nn.Embedding
            self.unaligned_embedder = mbnb.nn.Embedding(
                num_unaligned_tokens, embedding_dim
            )
    ...
    def before_step(self, it):
        if not self.component_gradient_boosting:
            return
        MIN_PROPORTIONAL_BOOST_LEVEL = 0.5
        MAX_MULTIPLIER = 100
        components = [
            list(self.contextual_embedder.parameters()),
        ...
        input_norm = torch.norm(
            torch.stack(
                [torch.norm(p.grad.detach(), 2) for p in self.input_blocks.parameters()]
            ),
            2,
        )
        output_norm = torch.norm(
            torch.stack(
                [
                    ...
                    for p in self.output_blocks.parameters()
                ]
            ),
            2,
        )
        ...
        for component in components:
            norm = torch.norm(
                torch.stack([torch.norm(p.grad.detach(), 2) for p in component]), 2
            )
            if norm < min_norm:
        ...
        with autocast(x.device.type, enabled=self.enable_fp16):
            ...
            if conditioning_free:
                ...
                if self.enable_unaligned_inputs:
                    ...
                    unaligned_h = self.unaligned_embedder(unaligned_input).permute(
                        0, 2, 1
                    )
                ...
                if tokens is not None:
                    ...
                    if self.training and self.nil_guidance_fwd_proportion > 0:
                        token_mask = clustered_mask(
                            self.nil_guidance_fwd_proportion,
                            tokens.shape,
                            tokens.device,
                            inverted=True,
                        )
                    ...
                    cond_time_emb = timestep_embedding(
                        torch.zeros_like(timesteps), code_emb.shape[1]
                    )  # This was something I was doing (adding timesteps into this computation), but removed on second thought. TODO: completely remove.
                    cond_time_emb = cond_time_emb.unsqueeze(-1).repeat(
                        1, 1, code_emb.shape[-1]
                    )
                    code_emb = self.conditioning_conv(
                        torch.cat([cond_emb, code_emb, cond_time_emb], dim=1)
                    )
                else:
            ...
            if self.training and self.unconditioned_percentage > 0:
                ...
                code_emb = torch.where(
                    unconditioned_batches,
                    self.unconditioned_embedding.repeat(x.shape[0], 1, 1),
                    code_emb,
                )
            ...
            for k, module in enumerate(self.input_blocks):
                if isinstance(module, nn.Conv1d):
                    ...
                    with autocast(
                        x.device.type, enabled=self.enable_fp16 and not first
                    ):
