...
class DietAttentionBlock(TimestepBlock):
    def __init__(self, in_dim, dim, heads, dropout):
        ...
        self.ff = FeedForward(
            dim, in_dim, mult=1, dropout=dropout, zero_init_output=True
        )
...
class TransformerDiffusionTTS(nn.Module):
    ...
    def __init__(
        ...
        self.conditioning_embedder = nn.Sequential(
            nn.Conv1d(in_channels, prenet_channels // 2, 3, padding=1, stride=2),
        ...
        self.layers = TimestepRotaryEmbedSequential(
            *[
                DietAttentionBlock(
                    model_channels, block_channels, block_channels // 64, dropout
                )
    ...
    def timestep_independent(
        self, codes, conditioning_input, expected_seq_len, prenet_latent=None
    ):
    ...
    def forward(
        ...
        blk_emb = torch.cat(
            [
                self.time_embed(timestep_embedding(timesteps, self.prenet_channels)),
                cond_emb,
                clvp_emb,
                type_emb,
            ],
