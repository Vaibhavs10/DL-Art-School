...
class Wav2VecMatcher(nn.Module):
    ...
    def __init__(
        self, model_dim, encoder_depth, decoder_depth, num_text_tokens=148, dropout=0.1
    ):
        super().__init__()

        WAV2VEC_CHANNELS = 1024
        self.conditioning_encoder = AudioMiniEncoder(
            1,
            model_dim,
            base_channels=32,
            depth=6,
    ...
    def get_grad_norm_parameter_groups(self):
        return {
            ...
            "heads": list(self.w2v_query_encoder.parameters())
            + list(self.w2v_value_encoder.parameters()),
            "minicoder": list(self.conditioning_encoder.parameters()),
        }

    def forward(
        self, text_tokens, conditioning_clip, w2v_logits, token_lengths, clip_lengths
    ):
        ...
        dec_inputs = torch.cat(
            [
                self.decoder_start_embedding.repeat(w2v_values.shape[0], 1, 1),
                w2v_values,
            ],
            dim=1,
        )
        ...
        mse_pad_loss = F.mse_loss(
            keys_uncompressed,
            self.decoder_stop_embedding.repeat(b * l, 1),
            reduction="none",
        ).sum(dim=-1)
        ...
        ce_loss = (
            ce_loss1 * loss_mask_collapsed + ce_loss2 * loss_mask_collapsed
        ).mean()
    ...
    def sample(
        self, text_tokens, conditioning_clip, w2v_logit_iterable, audio_clip_iterable
    ):
        text_emb = self.text_embedding(text_tokens)
        cond_emb = self.conditioning_encoder(conditioning_clip)
        enc_inputs = torch.cat([cond_emb.unsqueeze(1), text_emb], dim=1)
