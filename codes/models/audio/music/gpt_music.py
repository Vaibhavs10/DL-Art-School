...
class ConditioningEncoder(nn.Module):
    def __init__(self, spec_dim, embedding_dim, attn_blocks=6, num_attn_heads=4):
        ...
        attn = []
        self.init = nn.Conv1d(
            spec_dim, embedding_dim, kernel_size=3, stride=2, padding=1
        )
        for a in range(attn_blocks):
            attn.append(
                AttentionBlock(embedding_dim, num_attn_heads, do_activation=True)
            )
...
class UpperConditioningEncoder(nn.Module):
    def __init__(self, spec_dim, embedding_dim, attn_blocks=6, num_attn_heads=4):
        ...
        self.init = nn.Sequential(
            nn.Conv1d(
                spec_dim,
                min(spec_dim + 128, embedding_dim),
                kernel_size=3,
                stride=2,
                padding=1,
            ),
            nn.Conv1d(
                min(spec_dim + 128, embedding_dim),
                min(spec_dim + 256, embedding_dim),
                kernel_size=3,
                stride=2,
                padding=1,
            ),
            nn.Conv1d(
                min(spec_dim + 256, embedding_dim),
                min(spec_dim + 384, embedding_dim),
                kernel_size=3,
                stride=2,
                padding=1,
            ),
            nn.Conv1d(
                min(spec_dim + 384, embedding_dim),
                min(spec_dim + 512, embedding_dim),
                kernel_size=3,
                stride=2,
                padding=1,
            ),
...
class UpperQuantizer(nn.Module):
    def __init__(self, spec_dim, embedding_dim, num_tokens):
        super().__init__()
        attn = []
        ...
        self.encoder = nn.Sequential(
            ...
            ResBlock(
                edim(2), out_channels=embedding_dim, use_conv=True, dims=1, down=True
            ),
...
class GptMusicLower(nn.Module):
    def __init__(
        ...
        num_upper_vectors=32768,
        fp16=True,
        freeze_upper_until=0,
        num_vaes=4,
        vqargs={},
    ):
        super().__init__()
    ...
    def forward(self, mel, conditioning, return_latent=False):
        ...
        upper_vector = F.interpolate(
            upper_vector.permute(0, 2, 1), size=codes.shape[1], mode="linear"
        )
    ...
    def get_debug_values(self, step, __):
        ...
        if self.upper_quantizer.total_codes > 0:
            return {
                "histogram_upper_codes": self.upper_quantizer.codes[
                    : self.upper_quantizer.total_codes
                ]
            }
        else:
            return {}
...
class GptMusicUpper(nn.Module):
    def __init__(
        ...
        self.upper_quantizer = MusicQuantizer2(
            inp_channels=256,
            inner_dim=[
                dim,
                max(512, dim - 128),
            ...
            codevector_dim=dim,
            codebook_size=num_upper_vectors,
            codebook_groups=num_upper_groups,
        ...
        self.embeddings = nn.ModuleList(
            [
                mbnb.nn.Embedding(num_upper_vectors, dim // num_upper_groups)
                for _ in range(num_upper_groups)
            ]
        )
        self.heads = nn.ModuleList(
            [mbnb.nn.Linear(dim, num_upper_vectors) for _ in range(num_upper_groups)]
        )
...
def test_lower():
    model = GptMusicLower(
        ...
        layers=12,
        fp16=False,
        freeze_upper_until=1000,
        num_target_vectors=8192,
        num_upper_vectors=8192,
        num_vaes=4,
        vqargs={
            "positional_dims": 1,
            "channels": 64,
