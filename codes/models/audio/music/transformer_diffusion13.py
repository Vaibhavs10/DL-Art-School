...
class SubBlock(nn.Module):
    def __init__(self, inp_dim, contraction_dim, heads, dropout):
        ...
        self.register_buffer(
            "mask", build_local_attention_mask(n=6000, l=64), persistent=False
        )
        self.pos_bias = RelativeQKBias(l=64, max_positions=6000)
        ff_contract = contraction_dim // 2
        self.ff1 = nn.Sequential(
            nn.Conv1d(inp_dim + contraction_dim, ff_contract, kernel_size=1),
            nn.GroupNorm(8, ff_contract),
            cGLU(ff_contract),
        )
        self.ff2 = nn.Sequential(
            nn.Conv1d(
                inp_dim + contraction_dim * 3 // 2,
                ff_contract,
                kernel_size=3,
                padding=1,
            ),
    ...
    def forward(self, x):
        ah = self.dropout(
            self.attn(x, mask=self.mask, qk_bias=self.pos_bias(x.shape[-1]))
        )
...
class ConditioningEncoder(nn.Module):
    def __init__(
        ...
        self.resolution_embedding.weight.data.mul(
            0.1
        )  # Reduces the relative influence of this embedding from the start.
        for a in range(attn_blocks):
            attn.append(
                AttentionBlock(
                    hidden_dim, num_attn_heads, do_checkpoint=do_checkpointing
                )
            )
            attn.append(
                ResBlock(hidden_dim, dims=1, checkpointing_enabled=do_checkpointing)
            )
...
class TransformerDiffusion(nn.Module):
    ...
    def __init__(
        ...
        self.conditioning_encoder = ConditioningEncoder(
            ...
            cond_proj_dim,
            resolution_steps,
            num_attn_heads=model_channels // 64,
        )
        ...
        self.layers = TimestepEmbedSequential(
            *[
                ConcatAttentionBlock(
                    model_channels,
                    contraction_dim,
                    time_proj_dim * 3 + cond_proj_dim,
                    num_heads,
                    dropout,
                )
    ...
    def input_to_random_resolution_and_window(self, x, ts, diffuser):
        ...
        s_prior = F.interpolate(s, scale_factor=0.25, mode="nearest")
        s_prior = F.interpolate(
            s_prior, size=(s.shape[-1],), mode="linear", align_corners=True
        )
        ...
        self.preprocessed = (
            s_prior_diffused,
            t_prior,
            torch.tensor([resolution] * x.shape[0], dtype=torch.long, device=x.device),
        )
        return s
    ...
    def forward(
        ...
        """
        conditioning_input = (
            x_prior if conditioning_input is None else conditioning_input
        )
        ...
        if resolution is None:
            ...
            assert (
                x_prior is None
            ), "Provided prior will not be used, instead preprocessing output will be used."
            ...
            if prior_timesteps is None:
                ...
                prior_timesteps = torch.tensor(
                    [0], device=x.device
                )  # Assuming batch_size=1 for inference.
            x_prior = F.interpolate(
                x_prior, size=(x.shape[-1],), mode="linear", align_corners=True
            )
        assert torch.all(
            timesteps - prior_timesteps >= 0
        ), f"Prior timesteps should always be lower (more resolved) than input timesteps. {timesteps}, {prior_timesteps}"
        ...
        if self.training and self.unconditioned_percentage > 0:
            ...
            code_emb = torch.where(
                unconditioned_batches,
                self.unconditioned_embedding.repeat(code_emb.shape[0], 1),
                code_emb,
            )
    ...
    def get_grad_norm_parameter_groups(self):
        ...
        ff1 = list(
            itertools.chain.from_iterable(
                [lyr.block1.ff1.parameters() for lyr in self.layers]
                + [lyr.block1.ff2.parameters() for lyr in self.layers]
            )
        )
        ff2 = list(
            itertools.chain.from_iterable(
                [lyr.block2.ff1.parameters() for lyr in self.layers]
