...
class SelfClassifyingHead(nn.Module):
    def __init__(
        self, dim, classes, out_dim, head_depth, seq_len, dropout, init_temperature
    ):
        super().__init__()
    ...
    def forward(self, x, target):
        ...
        for i in range(self.seq_len):
            q, c = checkpoint(
                functools.partial(self.do_ar_step, used_codes=codes),
                torch.stack(stack, dim=1),
            )
            ...
            if len(results) > 0:
                worsen = (
                    F.mse_loss(outputs[-1], target, reduction="none").sum(-1)
                    < F.mse_loss(output, target, reduction="none").sum(-1)
                ).float()
                probabilistic_worsen = torch.rand_like(worsen) * worsen > 0.5
                output = output * probabilistic_worsen.unsqueeze(
                    -1
...
class VectorResBlock(nn.Module):
    def __init__(self, dim, dropout):
        ...
        self.ff = FeedForward(
            dim, mult=2, glu=True, dropout=dropout, zero_init_output=True
        )
...
class InstrumentQuantizer(nn.Module):
    ...
    def get_debug_values(self, step, __):
        if self.total_codes > 0:
            return {
                "histogram_codes": self.codes[: self.total_codes],
