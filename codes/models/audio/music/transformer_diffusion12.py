    ...
    def timestep_independent(self, prior, expected_seq_len):
        if self.new_code_expansion:
            prior = F.interpolate(
                ...
                mode="linear",
                align_corners=self.use_corner_alignment,
            ).permute(0, 2, 1)
...
class TransformerDiffusionWithQuantizer(nn.Module):
    def __init__(
        ...
        quantizer_codebook_groups=2,
        freeze_quantizer_until=20000,
        **kwargs,
    ):
        super().__init__()
        ...
        self.quantizer = MusicQuantizer2(
            inp_channels=kwargs["in_channels"],
            inner_dim=quantizer_dims,
            codevector_dim=quantizer_dims[0],
            codebook_size=quantizer_codebook_size,
            codebook_groups=quantizer_codebook_groups,
            max_gumbel_temperature=4,
    ...
    def update_for_step(self, step, *args):
        ...
        self.quantizer.quantizer.temperature = max(
            self.quantizer.max_gumbel_temperature
            * self.quantizer.gumbel_temperature_decay**qstep,
    ...
    def get_debug_values(self, step, __):
        if self.quantizer.total_codes > 0:
            return {
                "histogram_quant_codes": self.quantizer.codes[
                    : self.quantizer.total_codes
                ],
    ...
    def get_grad_norm_parameter_groups(self):
        attn1 = list(
            itertools.chain.from_iterable(
                [lyr.block1.attn.parameters() for lyr in self.diff.layers]
            )
        )
        attn2 = list(
            itertools.chain.from_iterable(
                [lyr.block2.attn.parameters() for lyr in self.diff.layers]
            )
        )
        ff1 = list(
            itertools.chain.from_iterable(
                [lyr.block1.ff.parameters() for lyr in self.diff.layers]
            )
        )
        ff2 = list(
            itertools.chain.from_iterable(
                [lyr.block2.ff.parameters() for lyr in self.diff.layers]
            )
        )
        blkout_layers = list(
            itertools.chain.from_iterable(
                [lyr.out.parameters() for lyr in self.diff.layers]
            )
        )
        groups = {
            "prenorms": list(
            ...
            "code_converters": list(self.diff.input_converter.parameters())
            + list(self.diff.code_converter.parameters()),
    ...
    def get_debug_values(self, step, __):
        if self.quantizers[0].total_codes > 0:
            ...
            for i in range(len(self.quantizers)):
                dbgs[f"histogram_quant{i}_codes"] = self.quantizers[i].codes[
                    : self.quantizers[i].total_codes
                ]
            return dbgs
        else:
            return {}
...
class TransformerDiffusionWithCheaterLatent(nn.Module):
    ...
    def forward(
        ...
        unused_parameters = []
        encoder_grad_enabled = (
            self.freeze_encoder_until is not None
            and self.internal_step > self.freeze_encoder_until
        )
...
def test_quant_model():
    ...
    # For music:
    model = TransformerDiffusionWithQuantizer(
        in_channels=256,
        model_channels=1536,
        contraction_dim=768,
        prenet_channels=1024,
    ...
    quant_weights = torch.load(
        "D:\\dlas\\experiments\\train_music_quant_r4\\models\\5000_generator.pth"
    )
...
def test_vqvae_model():
    ...
    # For music:
    model = TransformerDiffusionWithPretrainedVqvae(
        in_channels=100,
        out_channels=200,
        model_channels=1024,
        contraction_dim=512,
...
def test_multi_vqvae_model():
    ...
    # For music:
    model = TransformerDiffusionWithMultiPretrainedVqvae(
        in_channels=256,
        out_channels=512,
        model_channels=1024,
        contraction_dim=512,
...
def test_cheater_model():
    ...
    # For music:
    model = TransformerDiffusionWithCheaterLatent(
        in_channels=256,
        out_channels=512,
        model_channels=1024,
        contraction_dim=512,
    ...
    model.encoder.load_state_dict(
        torch.load(
            "../experiments/music_cheater_encoder_256.pth",
...
def extract_diff(in_f, out_f, remove_head=False):
    ...
    for k, v in p.items():
        if k.startswith("diff."):
            if remove_head and (
                k.startswith("diff.input_converter")
                or k.startswith("diff.code_converter")
            ):
                continue
