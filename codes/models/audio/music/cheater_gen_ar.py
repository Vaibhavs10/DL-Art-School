...
class ConditioningAR(nn.Module):
    ...
    def forward(
        self, cheater_codes, conditioning, code_lengths=None, return_latent=False
    ):
        unused_params = []
        ...
        if self.training and self.unconditioned_percentage > 0:
            unconditioned_batches = (
                torch.rand((cond.shape[0], 1), device=cond.device)
                < self.unconditioned_percentage
            )
            cond = torch.where(
                unconditioned_batches, self.cond_free_emb.repeat(cond.shape[0], 1), cond
            )
        ...
        targets = (
            cheater_codes  # Since we padded above by 1, the input alignment works.
        )
        ...
        if code_lengths is not None:
            mask = torch.arange(0, loss.shape[1], device=h.device).unsqueeze(0).repeat(
                loss.shape[0], 1
            ) < code_lengths.unsqueeze(1)
            loss = loss * mask
        loss = loss.mean()
...
def test_ar():
    model = ConditioningAR(512, 8, cond_free_percent=0.5)
    print_network(model)
