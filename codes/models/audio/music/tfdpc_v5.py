...
class SubBlock(nn.Module):
    def __init__(self, inp_dim, contraction_dim, heads, dropout, use_conv):
        ...
        if use_conv:
            ...
            self.ff = FeedForward(
                inp_dim + contraction_dim,
                dim_out=contraction_dim,
                mult=2,
                dropout=dropout,
            )
...
class ConcatAttentionBlock(TimestepBlock):
    def __init__(
        ...
        self.block2 = SubBlock(
            self.tdim + contraction_dim * 2, contraction_dim, heads, dropout, use_conv
        )
...
class TransformerDiffusionWithPointConditioning(nn.Module):
    def __init__(
        ...
        self.conditioning_encoder = ConditioningEncoder(
            ...
            model_channels,
            time_embed_dim,
            do_checkpointing=checkpoint_conditioning,
            time_proj=time_proj,
        )
    ...
    def process_conditioning(
        self, conditioning_input, time_emb, N, cond_start, cond_left, cond_right
    ):
        if self.training and self.regularization:
            ...
            for k in range(1, random.randint(2, 4)):
                ...
                tclip = random.randint(
                    1, min(conditioning_input.shape[-1] - tstart, 10)
                )
                conditioning_input[:, :, tstart : tstart + tclip] = 0

        if cond_left is None and self.new_cond:
            assert cond_start > 20 and (
                cond_start + N + 20 <= conditioning_input.shape[-1]
            ), f"{cond_start}, {N}, {conditioning_input.shape}"
            ...
            if self.training:
                ...
                if cond_left.shape[-1] > arb_context_cap and random.random() > 0.5:
                    cond_left = cond_left[:, :, -arb_context_cap:]
                if cond_right.shape[-1] > arb_context_cap and random.random() > 0.5:
                    cond_right = cond_right[:, :, :arb_context_cap]

        elif cond_left is None:
            assert (
                conditioning_input.shape[-1] - cond_start - N >= 0
            ), f"Some sort of conditioning misalignment, {conditioning_input.shape[-1], cond_start, N}"
            ...
            assert N > (
                MIN_MARGIN * 2 + 4
            ), f"Input size too small. Was {N} but requires at least {MIN_MARGIN*2+4}"
        ...
        cond = F.interpolate(
            cond_enc, size=(N,), mode="linear", align_corners=True
        ).permute(0, 2, 1)
        return cond
    ...
    def forward(
        ...
        if conditioning_free:
            ...
            cond = self.process_conditioning(
                conditioning_input,
                time_emb,
                x.shape[-1],
                cond_start,
            ...
            if self.training and self.unconditioned_percentage > 0:
                unconditioned_batches = (
                    torch.rand((cond.shape[0], 1, 1), device=cond.device)
                    < self.unconditioned_percentage
                )
                cond = torch.where(
                    unconditioned_batches,
                    self.unconditioned_embedding.repeat(cond.shape[0], 1, 1),
                    cond,
                )
...
def test_cheater_model():
    ...
    # For music:
    model = TransformerDiffusionWithPointConditioning(
        in_channels=256,
        out_channels=512,
        model_channels=1024,
        contraction_dim=512,
...
def inference_tfdpc5_with_cheater():
    with torch.no_grad():
        ...
        samples = {
            "electronica1": load_audio("Y:\\split\\yt-music-eval\\00001.wav", 22050),
            "electronica2": load_audio("Y:\\split\\yt-music-eval\\00272.wav", 22050),
            "e_guitar": load_audio("Y:\\split\\yt-music-eval\\00227.wav", 22050),
            "creep": load_audio(
                "Y:\\separated\\bt-music-3\\[2007] MTV Unplugged (Live) (Japan Edition)\\05 - Creep [Cover On Radiohead]\\00001\\no_vocals.wav",
                22050,
            ),
            "rock1": load_audio(
                "Y:\\separated\\bt-music-3\\2016 - Heal My Soul\\01 - Daze Of The Night\\00000\\no_vocals.wav",
                22050,
            ),
            "kiss": load_audio(
                "Y:\\separated\\bt-music-3\\KISS (2001) Box Set CD1\\02 Deuce (Demo Version)\\00000\\no_vocals.wav",
                22050,
            ),
            "purp": load_audio(
                "Y:\\separated\\bt-music-3\\Shades of Deep Purple\\11 Help (Alternate Take)\\00001\\no_vocals.wav",
                22050,
            ),
            "western_stars": load_audio(
                "Y:\\separated\\bt-music-3\\Western Stars\\01 Hitch Hikin'\\00000\\no_vocals.wav",
                22050,
            ),
            "silk": load_audio(
                "Y:\\separated\\silk\\MonstercatSilkShowcase\\890\\00007\\no_vocals.wav",
                22050,
            ),
            "long_electronica": load_audio(
                "C:\\Users\\James\\Music\\longer_sample.wav", 22050
            ),
        }
        for k, sample in samples.items():
            ...
            model.load_state_dict(
                torch.load(
                    "x:/dlas/experiments/train_music_cheater_gen_v3/models/59000_generator_ema.pth"
                )
            )

            from trainer.injectors.audio_injectors import TorchMelSpectrogramInjector

            spec_fn = TorchMelSpectrogramInjector(
                {
                    "n_mel_channels": 256,
                    "mel_fmax": 11000,
                    "filter_length": 16000,
            ...
            diffuser = SpacedDiffusion(
                use_timesteps=space_timesteps(4000, [128]),
            ...
            gen_cheater = diffuser.ddim_sample_loop(
                model,
                (1, 256, length),
                progress=True,
                model_kwargs={"true_cheater": ref_cheater},
            )
            ...
            wrap.load_state_dict(
                torch.load(
                    "x:/dlas/experiments/train_music_diffusion_tfd_cheater_from_scratch/models/56500_generator_ema.pth"
                )
            )
            cheater_to_mel = wrap.diff
            gen_mel = diffuser.ddim_sample_loop(
                cheater_to_mel,
            ...
            spectral_diffuser = SpacedDiffusion(
                use_timesteps=space_timesteps(4000, [32]),
                model_mean_type="epsilon",
                model_var_type="learned_range",
                loss_type="mse",
            ...
            gen_wav = spectral_diffuser.ddim_sample_loop(
                m2w, output_shape, model_kwargs={"codes": gen_mel_denorm}
            )
            ...
            torchaudio.save(
                f"results/tfdpc_v3/{k}.wav", gen_wav.squeeze(1).cpu(), 22050
            )
            torchaudio.save(
                f"results/tfdpc_v3/{k}_ref.wav", sample.unsqueeze(0).cpu(), 22050
            )
