...
def make_res_layer(block, inplanes, planes, blocks, stride=1, dilation=1):
    ...
    for i in range(1, blocks):
        layers.append(
            block(inplanes=inplanes, planes=planes, stride=1, dilation=dilation)
        )
...
class SpineNet(nn.Module):
    ...
    def __init__(
        ...
        assert (
            min(output_level) > 2 and max(output_level) < 8
    ...
    def _make_stem_layer(self, in_channels):
        ...
        if self._early_double_reduce:
            self.conv1 = ConvGnSilu(in_channels, 64, kernel_size=7, stride=2)
    ...
    def _make_endpoints(self):
        ...
        for block_spec in self._block_specs:
            if block_spec.is_output:
                in_channels = (
                    int(FILTER_SIZE_MAP[block_spec.level] * self._filter_size_scale) * 4
                )
                self.endpoint_convs[str(block_spec.level)] = ConvGnSilu(
                    in_channels,
    ...
    def _make_scale_permuted_network(self):
        ...
        for spec in self._block_specs:
            ...
            self.scale_permuted_blocks.append(
                make_res_layer(
                    spec.block_fn, in_channels, channels, self._block_repeats
                )
            )
    ...
    def forward(self, input):
        ...
        return tuple(
            [
                self.endpoint_convs[str(level)](output_feat[level])
...
class SpinenetWithLogits(SpineNet):
    def __init__(
        ...
        self.tail = nn.Sequential(
            ConvBnRelu(256, 128, kernel_size=1, activation=True, norm=True, bias=False),
            ConvBnRelu(128, 64, kernel_size=1, activation=True, norm=True, bias=False),
            ConvBnRelu(
                64, num_labels, kernel_size=1, activation=False, norm=False, bias=True
            ),
            nn.Softmax(dim=1),
        )
...
def register_spinenet(opt_net, opt):
    return SpineNet(
        str(opt_net["arch"]), in_channels=3, use_input_norm=opt_net["use_input_norm"]
    )


@register_model
def register_spinenet_with_logits(opt_net, opt):
    return SpinenetWithLogits(
        str(opt_net["arch"]),
        opt_net["output_to_attach"],
        opt_net["num_labels"],
        in_channels=3,
        use_input_norm=opt_net["use_input_norm"],
    )
