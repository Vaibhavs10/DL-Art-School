...
class VitLatent(nn.Module):
    def __init__(self, top_dim, hidden_dim, depth, dropout=0.1):
        super().__init__()
        self.upper = nn.Sequential(
            nn.Conv2d(3, top_dim, kernel_size=7, padding=3, stride=2),
            ResBlock(top_dim, use_conv=True, dropout=dropout),
            ResBlock(
                top_dim,
                out_channels=top_dim * 2,
                down=True,
                use_conv=True,
                dropout=dropout,
            ),
        ...
        self.mlp = nn.Sequential(
            mbnb.nn.Linear(hidden_dim, hidden_dim * 2),
