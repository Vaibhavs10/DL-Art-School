...
class ResidualDenseBlock(nn.Module):
    ...
    def __init__(
        self, mid_channels=64, growth_channels=32, embedding=False, init_weight=0.1
    ):
    ...
    def forward(self, x, emb):
        ...
        return self.normalize(x5 * 0.2 + x)
...
class RRDB(nn.Module):
    ...
    def __init__(self, mid_channels, growth_channels=32):
        ...
        self.normalize = nn.GroupNorm(num_groups=8, num_channels=mid_channels)
        self.residual_mult = nn.Parameter(torch.FloatTensor([0.1]))
...
class RRDBNet(nn.Module):
    ...
    def __init__(
        ...
        for m in [self.conv_body, self.conv_up1, self.conv_up2, self.conv_hr]:
            if m is not None:
    ...
    def forward(self, x, timesteps, low_res, correction_factors=None):
        ...
        if correction_factors is not None:
            correction_factors = correction_factors.view(x.shape[0], -1, 1, 1).repeat(
