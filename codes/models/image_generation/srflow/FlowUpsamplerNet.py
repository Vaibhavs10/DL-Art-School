...
class FlowUpsamplerNet(nn.Module):
    ...
    def arch_FlowStep(
        ...
        for k in range(K):
            position_name = self.get_position_name(
                H, opt_get(self.opt, ["networks", "generator", "flow_scale"])
            )
            if normOpt:
    ...
    def get_condAffSetting(self, opt, opt_get):
        ...
        condAff = (
            opt_get(opt, ["networks", "generator", "flow", "condFtAffine"]) or condAff
        )
        return condAff
    ...
    def arch_split(self, H, W, L, levels, opt, opt_get):
        ...
        if (
            opt_get(opt, ["networks", "generator", "flow", "split", "enable"])
            and L < levels - correction
        ):
            logs_eps = (
                opt_get(opt, ["networks", "generator", "flow", "split", "logs_eps"])
                or 0
            )
            consume_ratio = (
                opt_get(
                    opt, ["networks", "generator", "flow", "split", "consume_ratio"]
                )
                or 0.5
            )
            position_name = self.get_position_name(
            ...
            position = (
                position_name
                if opt_get(
                    opt, ["networks", "generator", "flow", "split", "conditional"]
                )
                else None
            )
            ...
            t = opt_get(
                opt, ["networks", "generator", "flow", "split", "type"], "Split2d"
            )

            if t == "Split2d":
                split = models.image_generation.srflow.Split.Split2d(
                    num_channels=self.C,
                    logs_eps=logs_eps,
                    position=position,
    ...
    def get_affineInCh(self, opt_get):
        affineInCh = (
            opt_get(self.opt, ["networks", "generator", "flow", "stackRRDB", "blocks"])
            or []
        )
    ...
    def decode(
        ...
        if (
            not opt_get(
                self.opt,
                ["networks", "generator", "flow", "levelConditional", "conditional"],
            )
            == True
        ):
