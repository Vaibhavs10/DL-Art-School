...
class RRDBNet(nn.Module):
    def __init__(
        self, in_nc, out_nc, nf, nb, gc=32, scale=4, initial_conv_stride=1, opt=None
    ):
        self.opt = opt
    ...
    def forward(self, x, get_steps=False):
        ...
        if self.scale >= 4:
            ...
            fea_up0_en = (
                opt_get(self.opt, ["networks", "generator", "flow", "fea_up0"]) or False
            )
            if fea_up0_en:
            ...
            fea_upn1_en = (
                opt_get(self.opt, ["networks", "generator", "flow", "fea_up-1"])
                or False
            )
            if fea_upn1_en:
...
class RRDBLatentWrapper(nn.Module):
    def __init__(
        ...
        fake_opt = {
            "networks": {
                "generator": {
                    "flow": {"stackRRDB": {"blocks": blocks}},
                    "rrdb_bypass": with_bypass,
                }
            }
        }
        ...
        self.postprocess = nn.Sequential(
            ...
            ConvGnLelu(
                out_dim, out_dim, kernel_size=1, bias=True, activation=False, norm=False
            ),
        )
...
def register_rrdb_latent_wrapper(opt_net, opt):
    return RRDBLatentWrapper(
        in_nc=opt_net["in_nc"],
        out_nc=opt_net["out_nc"],
        nf=opt_net["nf"],
        nb=opt_net["nb"],
        with_bypass=opt_net["with_bypass"],
        blocks=opt_net["blocks_for_latent"],
        scale=opt_net["scale"],
