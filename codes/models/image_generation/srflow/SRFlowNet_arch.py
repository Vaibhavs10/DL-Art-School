...
class SRFlowNet(nn.Module):
    def __init__(
        ...
        self.RRDB = RRDBNet(
            ...
            scale=scale,
            opt=opt,
            initial_conv_stride=initial_stride,
        )
        ...
        self.flowUpsamplerNet = FlowUpsamplerNet(
            (self.patch_sz, self.patch_sz, 3),
            hidden_channels,
            K,
            flow_coupling=opt["networks"]["generator"]["flow"]["coupling"],
            opt=opt,
        )
        self.i = 0
        self.dbg_logp = 0
    ...
    def forward(
        ...
        if not reverse:
            ...
            if z is None:
                ...
                lr_shape = [
                    d * self.opt["scale"] / self.flow_scale for d in lr.shape[2:]
                ]
                z = self.get_random_z(
    ...
    def normal_flow(
        ...
        if add_gt_noise:
            ...
            noiseQuant = opt_get(
                self.opt,
                ["networks", "generator", "flow", "augmentation", "noiseQuant"],
                True,
            )
            if noiseQuant:
        ...
        self.dbg_logp = -logp.mean().item() / float(np.log(2.0) * pixels)
        self.dbg_logdet = -logdet.mean().item() / float(np.log(2.0) * pixels)
    ...
    def rrdbPreprocessing(self, lr):
        ...
        if len(block_idxs) > 0:
            ...
            if (
                opt_get(
                    self.opt, ["networks", "generator", "flow", "stackRRDB", "concat"]
                )
                or False
            ):
