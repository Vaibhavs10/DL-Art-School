...
class ResidualDenseBlock(nn.Module):
    ...
    def __init__(self, mid_channels=64, growth_channels=32, init_weight=0.1):
...
class RRDBWithBypass(nn.Module):
    ...
    def __init__(
        ...
        growth_channels=32,
        reduce_to=None,
        randomly_add_noise_to_bypass=False,
    ):
    ...
    def forward(self, x):
        ...
        if self.randomly_add_bypass_noise and random.random() < 0.2:
            rnoise = torch.rand_like(bypass) * 0.02
    ...
    def forward(self, x, ref=None):
        ...
        if self.additive_mode == "additive":
            ...
            out = out - F.interpolate(
                out_pooled, scale_factor=self.scale, mode="nearest"
            )
            out = out + x_interp
...
def register_RRDBNetBypass(opt_net, opt):
    additive_mode = (
        opt_net["additive_mode"] if "additive_mode" in opt_net.keys() else "not"
    )
    output_mode = (
        opt_net["output_mode"] if "output_mode" in opt_net.keys() else "hq_only"
    )
    gc = opt_net["gc"] if "gc" in opt_net.keys() else 32
    initial_stride = (
        opt_net["initial_stride"] if "initial_stride" in opt_net.keys() else 1
    )
    ...
    return RRDBNet(
        in_channels=opt_net["in_nc"],
        out_channels=opt_net["out_nc"],
        mid_channels=opt_net["nf"],
