...
class FixupBasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, conv_create=conv3x3
    ):
...
class FixupResNet(nn.Module):
    def __init__(
        self, block, layers, upscale_applications=2, num_filters=64, inject_noise=False
    ):
        super(FixupResNet, self).__init__()
        self.inject_noise = inject_noise
        self.num_layers = sum(layers) + layers[-1] * (
        ...
        self.conv1 = nn.Conv2d(
            3, num_filters, kernel_size=5, stride=1, padding=2, bias=False
        )
        self.bias1 = nn.Parameter(torch.zeros(1))
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.layer1 = self._make_layer(block, num_filters, layers[0], stride=1)
        self.skip1 = nn.Conv2d(
            num_filters, 3, kernel_size=5, stride=1, padding=2, bias=False
        )
        ...
        if layers[1] > 0:
            ...
            self.layer2 = self._make_layer(
                block, int(self.nf2), layers[1], stride=1, conv_type=conv5x5
            )
