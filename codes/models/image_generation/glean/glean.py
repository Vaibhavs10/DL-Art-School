...
class GleanEncoderBlock(nn.Module):
    def __init__(self, nf, max_nf):
        ...
        self.process = nn.Sequential(
            ...
            ConvGnLelu(
                top_nf, top_nf, kernel_size=3, activation=True, norm=False, bias=False
            ),
        )
...
class GleanEncoder(nn.Module):
    def __init__(
        ...
        max_nf=512,
        reductions=4,
        latent_bank_blocks=7,
        latent_bank_latent_dim=512,
        input_dim=32,
        initial_stride=1,
    ):
        super().__init__()
        ...
        self.reducers = nn.ModuleList(
            [
                GleanEncoderBlock(min(nf * 2**i, max_nf), max_nf)
        ...
        self.latent_linear = EqualLinear(
            reducer_output_dim * reducer_output_nf,
...
class GleanDecoder(nn.Module):
    ...
    def __init__(self, nf, latent_bank_filters=[512, 256, 128]):
        ...
        self.decoder_blocks = nn.ModuleList(
            [
                ConvGnLelu(
                    decoder_block_shuffled_dims[i] + latent_bank_filters[i],
...
class GleanGenerator(nn.Module):
    def __init__(
        self,
        nf,
        latent_bank_pretrained_weights,
        latent_bank_max_dim=1024,
        gen_output_dim=256,
        encoder_rrdb_nb=6,
        latent_bank_latent_dim=512,
        input_dim=32,
        ...
        latent_blocks = int(
            math.log(gen_output_dim, 2)
        )  # From 4x4->gen_output_dim x gen_output_dim + initial styled conv
        encoder_reductions = int(math.log(after_stride_dim / 4, 2)) + 1
        self.encoder = GleanEncoder(
            nf,
            encoder_rrdb_nb,
            reductions=encoder_reductions,
            latent_bank_blocks=latent_blocks,
            latent_bank_latent_dim=latent_bank_latent_dim,
            input_dim=after_stride_dim,
        ...
        self.latent_bank = Stylegan2LatentBank(
            latent_bank_pretrained_weights,
            encoder_nf=nf,
            max_dim=latent_bank_max_dim,
            latent_dim=latent_bank_latent_dim,
            encoder_levels=encoder_reductions,
