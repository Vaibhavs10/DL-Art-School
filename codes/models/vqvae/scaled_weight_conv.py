...
class ScaledWeightConv(_ConvNd):
    ...
    def forward(self, input: Tensor, masks: dict = None) -> Tensor:
        ...
        weighted_convs = [
            self._weighted_conv_forward(input, self.weight * scale + shift)
...
class ScaledWeightConvTranspose(_ConvTransposeNd):
    def __init__(
        ...
        self.weight_scales = nn.ParameterList(
            [
                nn.Parameter(
                    torch.ones(in_channels, out_channels, kernel_size, kernel_size)
                )
                for _ in range(breadth)
            ]
        )
        self.shifts = nn.ParameterList(
            [
                nn.Parameter(
                    torch.zeros(in_channels, out_channels, kernel_size, kernel_size)
                )
                for _ in range(breadth)
            ]
        )
    ...
    def forward(
        self, input: Tensor, masks: dict = None, output_size: Optional[List[int]] = None
    ) -> Tensor:
        if masks is None:
            # An alternate "mode" of operation is the masks are injected as parameters.
            assert hasattr(self, "masks")
        ...
        # good at all, this can be made more efficient by performing a single conv pass with multiple masks.
        weighted_convs = [
            self._conv_transpose_forward(
                input, self.weight * scale + shift, output_size
            )
            for scale, shift in zip(self.weight_scales, self.shifts)
        ]
        weighted_convs = torch.stack(weighted_convs, dim=-1)
...
def create_wrapped_conv_transpose_from_template(conv: nn.Conv2d, breadth: int):
    wrapped = ScaledWeightConvTranspose(
        conv.in_channels,
        conv.out_channels,
        conv.kernel_size,
        conv.stride,
